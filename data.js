const DATA_BANK = {
  "games": {
    "seen_jeem": {
      "name": "سين جيم",
      "description": "أسئلة ثقافية سعودية متنوعة",
      "totalQuestions": 216,
      "questions": [
        {
          "id": 1,
          "difficulty": "CLASS_200",
          "question": "مين هو مؤسس المملكة العربية السعودية اللي وحّد البلاد؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الملك عبدالعزيز بن عبدالرحمن آل سعود",
          "hint": "وحّد الجزيرة العربية وأسّس المملكة عام 1932م",
          "points": 200
        },
        {
          "id": 2,
          "difficulty": "CLASS_200",
          "question": "في أي سنة تأسّست المملكة العربية السعودية رسمياً؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "1932م",
          "hint": "السنة اللي أُعلن فيها اسم المملكة رسمياً",
          "points": 200
        },
        {
          "id": 3,
          "difficulty": "CLASS_400",
          "question": "وش اسم المنطقة اللي اكتُشف فيها أول بئر نفط تجاري في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الدمّام - بئر رقم 7",
          "hint": "المنطقة الشرقية، والبئر اشتهرت برقمها",
          "points": 400
        },
        {
          "id": 4,
          "difficulty": "CLASS_200",
          "question": "مين هو الملك الحالي للمملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الملك سلمان بن عبدالعزيز آل سعود",
          "hint": "تولّى الحكم عام 2015م",
          "points": 200
        },
        {
          "id": 5,
          "difficulty": "CLASS_400",
          "question": "وش يمثّل السيف والنخلة اللي في علم المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "السيف يمثّل القوة والعدالة، والنخلة تمثّل الرخاء والنماء",
          "hint": "شعار الدولة على خلفية خضراء",
          "points": 400
        },
        {
          "id": 6,
          "difficulty": "CLASS_200",
          "question": "وش اسم عاصمة المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1586724237569-f3d0c1dee8c6?w=640&q=80",
          "correctAnswer": "الرياض",
          "hint": "أكبر مدينة في المملكة",
          "points": 200
        },
        {
          "id": 7,
          "difficulty": "CLASS_600",
          "question": "في أي عام اكتمل فتح الحجاز وضمّه للمملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1564769625905-50e93615e769?w=640&q=80",
          "correctAnswer": "1925م",
          "hint": "قبل إعلان تأسيس المملكة بسبع سنوات",
          "points": 600
        },
        {
          "id": 8,
          "difficulty": "CLASS_400",
          "question": "وش اسم المشروع الضخم اللي يُبنى على ساحل البحر الأحمر وهو مدينة المستقبل؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/neom.com",
          "correctAnswer": "نيوم",
          "hint": "من أبرز مشاريع رؤية 2030",
          "points": 400
        },
        {
          "id": 9,
          "difficulty": "CLASS_200",
          "question": "وش اسم رؤية التحوّل الوطني اللي أطلقها ولي العهد الأمير محمد بن سلمان؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "رؤية 2030",
          "hint": "سنة مستهدفة لتحقيق أهداف التنمية",
          "points": 200
        },
        {
          "id": 10,
          "difficulty": "CLASS_600",
          "question": "مين أوّل ملك سعودي زار الولايات المتحدة الأمريكية رسمياً؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الملك فيصل بن عبدالعزيز",
          "hint": "الملك اللي كان من أبرز الزعماء العرب في القرن العشرين",
          "points": 600
        },
        {
          "id": 11,
          "difficulty": "CLASS_200",
          "question": "وش اسم التطبيق الحكومي السعودي اللي تقدر تسوّي فيه معاملاتك الحكومية كلها؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "أبشر",
          "hint": "تطبيق وزارة الداخلية",
          "points": 200
        },
        {
          "id": 12,
          "difficulty": "CLASS_400",
          "question": "وش اسم شركة الاتصالات السعودية الأولى واللي رمزها ثلاث حروف إنجليزية؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/stc.com.sa",
          "correctAnswer": "STC - شركة الاتصالات السعودية",
          "hint": "أكبر شركة اتصالات في المملكة",
          "points": 400
        },
        {
          "id": 13,
          "difficulty": "CLASS_200",
          "question": "وش اسم تطبيق الصحة السعودي اللي كان مهم جداً في وقت كورونا؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "توكّلنا",
          "hint": "يظهر فيه تصريح التنقل والتطعيم",
          "points": 200
        },
        {
          "id": 14,
          "difficulty": "CLASS_400",
          "question": "وش اسم أكبر شركة نفط في العالم واللي مقرّها في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/aramco.com",
          "correctAnswer": "أرامكو السعودية",
          "hint": "اختصار للاسم القديم: Arabian American Oil Company",
          "points": 400
        },
        {
          "id": 15,
          "difficulty": "CLASS_600",
          "question": "وش اسم مدينة تقنية المعلومات اللي أُسّست بالقرب من الرياض وتُسمّى 'وادي السيليكون السعودي'؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مدينة الملك عبدالعزيز للعلوم والتقنية (كاكست)",
          "hint": "مدينة علمية بحثية حكومية",
          "points": 600
        },
        {
          "id": 16,
          "difficulty": "CLASS_200",
          "question": "وش اسم الأكلة السعودية الشعبية المعروفة اللي تتكوّن من أرز وجوز ولحم أو دجاج؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=640&q=80",
          "correctAnswer": "الكبسة",
          "hint": "الأكلة الوطنية غير الرسمية للمملكة",
          "points": 200
        },
        {
          "id": 17,
          "difficulty": "CLASS_200",
          "question": "وش اسم اللبس الرجالي التقليدي الأبيض اللي يلبسه السعوديون؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الثوب",
          "hint": "يُلبس مع الغترة أو الشماغ",
          "points": 200
        },
        {
          "id": 18,
          "difficulty": "CLASS_400",
          "question": "وش اسم الرياضة الشعبية اللي يتدرّب فيها السعوديون على تطيير الطيور للصيد؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الصقارة",
          "hint": "من التراث العربي الأصيل وأُدرجت في قائمة اليونسكو",
          "points": 400
        },
        {
          "id": 19,
          "difficulty": "CLASS_200",
          "question": "وش اسم الرقصة الشعبية السعودية اللي تُؤدَّى في المناسبات الوطنية والأعراس؟",
          "mediaType": "video",
          "mediaUrl": "https://www.youtube.com/watch?v=7k5FTMYBZG4",
          "correctAnswer": "العرضة السعودية",
          "hint": "رقصة بالسيوف على إيقاع الطبول",
          "points": 200
        },
        {
          "id": 20,
          "difficulty": "CLASS_400",
          "question": "في القهوة العربية السعودية، وش الخلطة اللي تميّزها عن غيرها؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=640&q=80",
          "correctAnswer": "الزعفران والهيل (الكردمون)",
          "hint": "تلوّنها أصفر أو خضري وريحتها عطرية",
          "points": 400
        },
        {
          "id": 21,
          "difficulty": "CLASS_200",
          "question": "وش اسم المسلسل السعودي الكوميدي الشهير اللي استمر لأكثر من 20 سنة في رمضان؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "طاش ما طاش",
          "hint": "يمثّل فيه ناصر القصبي وعبدالله السدحان",
          "points": 200
        },
        {
          "id": 22,
          "difficulty": "CLASS_200",
          "question": "مين الممثل السعودي اللي يُقدّم برنامج سيلفي ويشتهر بأدوار الكوميديا؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ناصر القصبي",
          "hint": "قدّم أيضاً طاش ما طاش مع السدحان",
          "points": 200
        },
        {
          "id": 23,
          "difficulty": "CLASS_400",
          "question": "وش اسم المسلسل السعودي اللي تدور أحداثه حول عائلة في المنطقة الغربية وعُرض على شاشة MBC؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mbc.net",
          "correctAnswer": "عائلة الحاج متعب",
          "hint": "مسلسل اجتماعي يعكس حياة الأسرة الحجازية",
          "points": 400
        },
        {
          "id": 24,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول مسلسل سعودي يُبثّ على منصة نتفليكس؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/netflix.com",
          "correctAnswer": "جن",
          "hint": "مسلسل خيال علمي رعب سعودي أردني",
          "points": 600
        },
        {
          "id": 25,
          "difficulty": "CLASS_400",
          "question": "مين الممثلة السعودية اللي تعتبر من أبرز نجوم الدراما الخليجية وظهرت في مسلسل 'سمرة'؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "إلهام علي",
          "hint": "من أيقونات الجمال والتمثيل في المنطقة",
          "points": 400
        },
        {
          "id": 26,
          "difficulty": "CLASS_200",
          "question": "وش اسم شبكة القنوات الإسلامية اللي مقرّها في الرياض وشعارها هلال؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "شبكة المجد",
          "hint": "تضم قنوات قرآن كريم، علمية، وثائقية، وأطفال",
          "points": 200
        },
        {
          "id": 27,
          "difficulty": "CLASS_400",
          "question": "وش اسم قناة الأطفال التابعة لشبكة المجد؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "قناة مجد للأطفال",
          "hint": "برامجها تعليمية وإسلامية موجهة للصغار",
          "points": 400
        },
        {
          "id": 28,
          "difficulty": "CLASS_400",
          "question": "وش اسم البرنامج الديني الشهير على شبكة المجد اللي يُجيب على أسئلة المشاهدين الشرعية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نور على الدرب",
          "hint": "برنامج فتاوى مباشر يُجيب فيه العلماء على أسئلة المستمعين",
          "points": 400
        },
        {
          "id": 29,
          "difficulty": "CLASS_600",
          "question": "في أي عام تأسّست شبكة المجد التلفزيونية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "2002م",
          "hint": "في مطلع الألفية الثالثة",
          "points": 600
        },
        {
          "id": 30,
          "difficulty": "CLASS_400",
          "question": "وش اسم القناة التابعة لشبكة المجد المخصصة للبرامج العلمية والوثائقية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مجد العلمية",
          "hint": "تعنى بالبرامج الوثائقية والعلمية الإسلامية",
          "points": 400
        },
        {
          "id": 31,
          "difficulty": "CLASS_200",
          "question": "وش اسم النادي اللي يُلقّب بـ'الهلال' ويُعتبر الأكثر ألقاباً في الدوري السعودي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نادي الهلال السعودي",
          "hint": "شعاره هلال أزرق على خلفية بيضاء",
          "points": 200
        },
        {
          "id": 32,
          "difficulty": "CLASS_200",
          "question": "وش اسم النادي السعودي اللي انضم إليه النجم البرتغالي كريستيانو رونالدو عام 2023؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نادي النصر",
          "hint": "ألوانه أصفر وأزرق",
          "points": 200
        },
        {
          "id": 33,
          "difficulty": "CLASS_400",
          "question": "وش اسم النجم الفرنسي اللي انضم لنادي الهلال السعودي عام 2023؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/alhilal.com",
          "correctAnswer": "كريم بنزيمة",
          "hint": "الفائز بجائزة الكرة الذهبية 2022",
          "points": 400
        },
        {
          "id": 34,
          "difficulty": "CLASS_200",
          "question": "كم عدد الأندية في دوري روشن للمحترفين (الدوري السعودي للمحترفين)؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "18 نادياً",
          "hint": "الدوري من أكبر دوريات آسيا",
          "points": 200
        },
        {
          "id": 35,
          "difficulty": "CLASS_600",
          "question": "وش اسم الملعب اللي يُقام فيه مباريات نادي الاتحاد في جدة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ملعب الأمير عبدالله الفيصل",
          "hint": "سُمّي باسم أحد أمراء المملكة",
          "points": 600
        },
        {
          "id": 36,
          "difficulty": "CLASS_200",
          "question": "في أي منطقة إدارية تقع محافظة الزلفي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "منطقة الرياض",
          "hint": "تقع شمال غرب العاصمة الرياض",
          "points": 200
        },
        {
          "id": 37,
          "difficulty": "CLASS_400",
          "question": "وش أبرز محصول زراعي تشتهر فيه محافظة الزلفي؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1605027990121-cbae9e0642df?w=640&q=80",
          "correctAnswer": "التمور (النخيل)",
          "hint": "من أشهر مناطق إنتاج التمر في المملكة",
          "points": 400
        },
        {
          "id": 38,
          "difficulty": "CLASS_600",
          "question": "وش اسم البحيرة الموسمية الشهيرة القريبة من الزلفي واللي تتحوّل لمتنزه طبيعي بعد الأمطار؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "بحيرة الزلفي (الأثل أو الروضة)",
          "hint": "تملأها مياه الأمطار وتجذب الزوار",
          "points": 600
        },
        {
          "id": 39,
          "difficulty": "CLASS_400",
          "question": "بكم تبعد محافظة الزلفي تقريباً عن مدينة الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "حوالي 270 كيلومتر شمال غرب الرياض",
          "hint": "ساعتين ونص تقريباً بالسيارة من الرياض",
          "points": 400
        },
        {
          "id": 40,
          "difficulty": "CLASS_600",
          "question": "وش اسم جامعة الزلفي اللي أُسّست لخدمة المنطقة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "جامعة المجمعة (فرع الزلفي)",
          "hint": "جامعة حكومية تخدم محافظات المنطقة الوسطى",
          "points": 600
        },
        {
          "id": 41,
          "difficulty": "CLASS_200",
          "question": "وش اسم الشركة اليابانية اللي تصنع سيارة 'كامري' و'لاند كروزر'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/toyota.com",
          "correctAnswer": "تويوتا",
          "hint": "شعارها ثلاث بيضاويات متداخلة",
          "points": 200
        },
        {
          "id": 42,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة السيارات الألمانية الفاخرة اللي شعارها نجمة ثلاثية الرؤوس؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mercedes-benz.com",
          "correctAnswer": "مرسيدس بنز",
          "hint": "من أقدم وأشهر شركات السيارات في العالم",
          "points": 200
        },
        {
          "id": 43,
          "difficulty": "CLASS_400",
          "question": "وش اسم السيارة الإيطالية الحمراء الأسرع واللي شعارها حصان أسود؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/ferrari.com",
          "correctAnswer": "فيراري",
          "hint": "مقرّها مدينة مارانيلو الإيطالية",
          "points": 400
        },
        {
          "id": 44,
          "difficulty": "CLASS_400",
          "question": "وش اسم الشركة البريطانية الفاخرة جداً اللي يُقال إنها 'أفضل سيارات العالم' وشعارها تمثال فتاة؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/rolls-roycemotorcars.com",
          "correctAnswer": "رولز رويس",
          "hint": "شعارها 'روح النشوة' (Spirit of Ecstasy)",
          "points": 400
        },
        {
          "id": 45,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول سيارة كهربائية سعودية أُعلن عنها من مشاريع رؤية 2030؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "سيارة سير (CEER)",
          "hint": "شراكة سعودية مع شركة BMW لتصنيعها",
          "points": 600
        },
        {
          "id": 46,
          "difficulty": "CLASS_200",
          "question": "وش اسم سلسلة الوجبات السريعة الأمريكية اللي شعارها حرف M ذهبي؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mcdonalds.com",
          "correctAnswer": "ماكدونالدز",
          "hint": "أشهر سلسلة برغر في العالم",
          "points": 200
        },
        {
          "id": 47,
          "difficulty": "CLASS_400",
          "question": "وش اسم سلسلة المطاعم السعودية المحلية المشهورة بالبرغر واللي تنافس الشركات العالمية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "كودو (KUDU)",
          "hint": "سلسلة سعودية 100% بشعار وعل",
          "points": 400
        },
        {
          "id": 48,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة التوصيل والتسوق الإلكتروني السعودية اللي شعارها سهم برتقالي؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/amazon.com",
          "correctAnswer": "أمازون السعودية (سابقاً سوق.كوم)",
          "hint": "اشترتها أمازون عالمياً وغيّرت اسمها",
          "points": 200
        },
        {
          "id": 49,
          "difficulty": "CLASS_200",
          "question": "وش معنى كلمة 'زبالة' باللهجة السعودية الدارجة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "القمامة أو النفايات",
          "hint": "الكلمة اللي تقولها لما تبي تشيل الأوساخ",
          "points": 200
        },
        {
          "id": 50,
          "difficulty": "CLASS_400",
          "question": "وش معنى كلمة 'يهوبي' باللهجة النجدية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "رجل ضخم أو قوي الجسم",
          "hint": "وصف لشخص كبير الحجم وقوي",
          "points": 400
        },
        {
          "id": 51,
          "difficulty": "CLASS_200",
          "question": "وش اسم أقدس مدينتين في الإسلام واللي تقعان في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1564769625905-50e93615e769?w=640&q=80",
          "correctAnswer": "مكة المكرمة والمدينة المنورة",
          "hint": "فيهما الحرمين الشريفين",
          "points": 200
        },
        {
          "id": 52,
          "difficulty": "CLASS_400",
          "question": "في أي عام وُلد الملك عبدالعزيز مؤسس المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "1876م",
          "hint": "في القرن التاسع عشر الميلادي",
          "points": 400
        },
        {
          "id": 53,
          "difficulty": "CLASS_600",
          "question": "وش اسم المعركة اللي استعاد فيها الملك عبدالعزيز الرياض من آل رشيد؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "معركة المصمك (فتح الرياض)",
          "hint": "حدثت عام 1902م في قصر المصمك",
          "points": 600
        },
        {
          "id": 54,
          "difficulty": "CLASS_400",
          "question": "وش اسم القصر التاريخي اللي يرمز لاسترداد الرياض في عهد الملك عبدالعزيز؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1588001400947-6385aef4ab0e?w=640&q=80",
          "correctAnswer": "قصر المصمك",
          "hint": "موجود في وسط الرياض القديم ويعمل متحفاً",
          "points": 400
        },
        {
          "id": 55,
          "difficulty": "CLASS_200",
          "question": "وش اسم أطول برج في المملكة العربية السعودية والموجود في جدة؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1578895102073-a1481ebde81b?w=640&q=80",
          "correctAnswer": "برج جدة (المملكة)",
          "hint": "يُنافس على لقب أطول برج في العالم",
          "points": 200
        },
        {
          "id": 56,
          "difficulty": "CLASS_400",
          "question": "في أي عهد ملكي اكتُشف النفط في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "عهد الملك عبدالعزيز بن عبدالرحمن (1938م)",
          "hint": "في الثلاثينيات الميلادية",
          "points": 400
        },
        {
          "id": 57,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول رئيس وزراء معيّن في تاريخ المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الأمير فيصل بن عبدالعزيز (لاحقاً أصبح ملكاً)",
          "hint": "عُيِّن في الخمسينيات الميلادية",
          "points": 600
        },
        {
          "id": 58,
          "difficulty": "CLASS_600",
          "question": "وش اسم العملة السعودية قبل توحيد العملة بالريال؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الجنيه الذهبي السعودي",
          "hint": "كانت مقدّرة بالذهب",
          "points": 600
        },
        {
          "id": 59,
          "difficulty": "CLASS_400",
          "question": "وش اسم الموقع التاريخي الأثري في العُلا واللي أُدرج في قائمة التراث العالمي؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1548199569-3e1c6aa8f469?w=640&q=80",
          "correctAnswer": "العُلا - مدائن صالح (الحِجر)",
          "hint": "مدينة الأنباط المنحوتة في الصخر",
          "points": 400
        },
        {
          "id": 60,
          "difficulty": "CLASS_200",
          "question": "وش اسم الجهة الحكومية المسؤولة عن الهوية الرقمية وخدمات التوثيق في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "النفاذ الوطني الموحّد",
          "hint": "تسجيل الدخول الموحّد للخدمات الحكومية",
          "points": 200
        },
        {
          "id": 61,
          "difficulty": "CLASS_200",
          "question": "وش اسم منصة التواصل الاجتماعي السعودية الحكومية للمهنيين اللي تنافس لينكدإن؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/linkedin.com",
          "correctAnswer": "منصة مسار",
          "hint": "منصة التوظيف والتطوير المهني في المملكة",
          "points": 200
        },
        {
          "id": 62,
          "difficulty": "CLASS_400",
          "question": "وش اسم منصة التعلم الإلكتروني الحكومية السعودية للطلاب والمعلمين؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "منصة مدرستي",
          "hint": "استُخدمت بشكل كثيف خلال جائحة كورونا",
          "points": 400
        },
        {
          "id": 63,
          "difficulty": "CLASS_600",
          "question": "وش اسم الشركة السعودية الناشئة اللي طوّرت روبوت يتحدث العربية ويُسمّى 'ريم'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/aramco.com",
          "correctAnswer": "شركة SDAIA (هيئة البيانات والذكاء الاصطناعي)",
          "hint": "جهة حكومية تطوّر الذكاء الاصطناعي في المملكة",
          "points": 600
        },
        {
          "id": 64,
          "difficulty": "CLASS_400",
          "question": "وش اسم الجامعة السعودية المتخصصة في العلوم والتقنية والمعروفة بـ KAUST؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "جامعة الملك عبدالله للعلوم والتقنية",
          "hint": "موقعها على ساحل البحر الأحمر في ثول",
          "points": 400
        },
        {
          "id": 65,
          "difficulty": "CLASS_200",
          "question": "وش اسم تطبيق التوصيل السعودي الشهير باللون الأخضر واللي ينافس كريم وأوبر؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/careem.com",
          "correctAnswer": "كريم",
          "hint": "أُسِّس في دبي وانتشر بالسعودية واشترته أوبر",
          "points": 200
        },
        {
          "id": 66,
          "difficulty": "CLASS_200",
          "question": "وش اسم مهرجان الترفيه السعودي الكبير اللي يُقام سنوياً في الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "موسم الرياض",
          "hint": "يضم فعاليات ترفيهية ضخمة لأشهر عدة",
          "points": 200
        },
        {
          "id": 67,
          "difficulty": "CLASS_400",
          "question": "وش اسم أكبر مشروع ترفيهي في الشرق الأوسط اللي يُبنى في القدية قرب الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "مشروع القدية",
          "hint": "سيضم ملاهي وملاعب رياضية ومنتجعات",
          "points": 400
        },
        {
          "id": 68,
          "difficulty": "CLASS_200",
          "question": "وش اسم الأديب والشاعر السعودي الشهير اللي كان وزيراً وكاتباً وشاعراً في آنٍ واحد؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "غازي القصيبي",
          "hint": "كان وزير الصحة ثم العمل، وله روايات وقصائد شهيرة",
          "points": 200
        },
        {
          "id": 69,
          "difficulty": "CLASS_400",
          "question": "وش اسم أشهر مهرجان ثقافي سنوي يُقام في الجنادرية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مهرجان الجنادرية للتراث والثقافة",
          "hint": "يُقام شمال الرياض ويعرض تراث مناطق المملكة",
          "points": 400
        },
        {
          "id": 70,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول دار أوبرا في المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "دار الأوبرا في نيوم (مشروع قيد الإنشاء)",
          "hint": "جزء من مشاريع نيوم على ساحل البحر الأحمر",
          "points": 600
        },
        {
          "id": 71,
          "difficulty": "CLASS_200",
          "question": "وش اسم المسلسل السعودي الكوميدي اللي بطله 'حمد' ويتكلم عن حياة موظف؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mbc.net",
          "correctAnswer": "سيلفي",
          "hint": "يُقدّمه ناصر القصبي على MBC",
          "points": 200
        },
        {
          "id": 72,
          "difficulty": "CLASS_400",
          "question": "مين مخرج فيلم 'نورة' السعودي الذي عُرض في مهرجان كان؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "توفيق الزايدي",
          "hint": "فيلم حاز على اهتمام دولي كبير",
          "points": 400
        },
        {
          "id": 73,
          "difficulty": "CLASS_200",
          "question": "وش اسم المسلسل السعودي الكوميدي عن أسرة متوسطة الحال ويعرض في رمضان كل سنة؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mbc.net",
          "correctAnswer": "سمبوسة أو شاهد أو باب الحارة (خليجي)",
          "hint": "مسلسلات الكوميديا العائلية الرمضانية",
          "points": 200
        },
        {
          "id": 74,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول فيلم سعودي طويل يُعرض تجارياً في دور السينما السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "فيلم وجدة (2012) - رغم أن عرضه التجاري جاء لاحقاً",
          "hint": "فيلم للمخرجة هيفاء المنصور عن فتاة تحلم بدراجة",
          "points": 600
        },
        {
          "id": 75,
          "difficulty": "CLASS_400",
          "question": "وش اسم البرنامج الرمضاني الترفيهي السعودي المنافس اللي تشارك فيه مشاهير وفناّنون؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mbc.net",
          "correctAnswer": "برنامج رامز في الغابة أو Arab Idol",
          "hint": "برامج المنافسة الرمضانية الشهيرة على MBC",
          "points": 400
        },
        {
          "id": 76,
          "difficulty": "CLASS_200",
          "question": "وش اسم قناة الأخبار السعودية الرسمية التابعة للدولة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "قناة السعودية (التلفزيون السعودي)",
          "hint": "البث الرسمي للدولة منذ الستينيات",
          "points": 200
        },
        {
          "id": 77,
          "difficulty": "CLASS_600",
          "question": "في أي سنة بدأ البث التلفزيوني الرسمي في المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "1965م",
          "hint": "في عهد الملك فيصل رحمه الله",
          "points": 600
        },
        {
          "id": 78,
          "difficulty": "CLASS_400",
          "question": "وش اسم قناة شبكة المجد المتخصصة في القرآن الكريم والتلاوات؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "قناة مجد القرآن الكريم",
          "hint": "تبثّ تلاوات وتفسيراً ودروساً قرآنية",
          "points": 400
        },
        {
          "id": 79,
          "difficulty": "CLASS_200",
          "question": "وش اسم البرنامج الديني اللي يُبثّ على شبكة المجد خلال شهر رمضان ويجيب على أسئلة المشاهدين؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "برنامج الفتاوى المباشر",
          "hint": "يستقبل أسئلة الجمهور ويردّ عليها العلماء",
          "points": 200
        },
        {
          "id": 80,
          "difficulty": "CLASS_400",
          "question": "وش اسم قناة شبكة المجد المخصصة للبرامج الحضارية والتاريخية الإسلامية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مجد الحضارة",
          "hint": "تعرض برامج عن الحضارة والتاريخ الإسلامي",
          "points": 400
        },
        {
          "id": 81,
          "difficulty": "CLASS_600",
          "question": "وش اسم الشخصية السعودية البارزة اللي أسّست شبكة المجد التلفزيونية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الأمير طلال بن عبدالعزيز بن مساعد آل سعود",
          "hint": "أمير سعودي أسّس الشبكة عام 2002 لخدمة الإسلام والمسلمين",
          "points": 600
        },
        {
          "id": 82,
          "difficulty": "CLASS_400",
          "question": "وش اسم قناة المجد المتخصصة في البرامج الاجتماعية والأسرية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مجد الاجتماعية",
          "hint": "قناة تهتم بقضايا المرأة والأسرة المسلمة",
          "points": 400
        },
        {
          "id": 83,
          "difficulty": "CLASS_200",
          "question": "وش اسم أكثر نادي حقّق ألقاباً في تاريخ الدوري السعودي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نادي الهلال",
          "hint": "يُلقّب بـ'سيد الكرة السعودية'",
          "points": 200
        },
        {
          "id": 84,
          "difficulty": "CLASS_400",
          "question": "وش اسم النادي السعودي اللي فاز ببطولة دوري أبطال آسيا أكثر من مرة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نادي الهلال (4 مرات)",
          "hint": "النادي الأكثر تتويجاً آسيوياً",
          "points": 400
        },
        {
          "id": 85,
          "difficulty": "CLASS_200",
          "question": "وش ألوان نادي الاتحاد السعودي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الأصفر والأسود",
          "hint": "نادي جدة المشهور",
          "points": 200
        },
        {
          "id": 86,
          "difficulty": "CLASS_600",
          "question": "وش اسم الحارس السعودي اللي صدّ ضربات الأرجنتين في مونديال 2022 وصار نجم المباراة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "محمد العويس",
          "hint": "حارس مرمى نادي الهلال والمنتخب السعودي",
          "points": 600
        },
        {
          "id": 87,
          "difficulty": "CLASS_400",
          "question": "وش نتيجة مباراة السعودية والأرجنتين في كأس العالم 2022؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "فازت السعودية 2-1",
          "hint": "مفاجأة كأس العالم 2022",
          "points": 400
        },
        {
          "id": 88,
          "difficulty": "CLASS_200",
          "question": "وش اسم الملعب الرئيسي للمنتخب السعودي في الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1586724237569-f3d0c1dee8c6?w=640&q=80",
          "correctAnswer": "ملعب الملك فهد الدولي",
          "hint": "يتّسع لأكثر من 68 ألف متفرج",
          "points": 200
        },
        {
          "id": 89,
          "difficulty": "CLASS_600",
          "question": "وش اسم اللاعب السعودي اللي سجّل أكثر هدف في تاريخ الدوري السعودي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ماجد عبدالله",
          "hint": "أسطورة الهلال والمنتخب في الثمانينيات",
          "points": 600
        },
        {
          "id": 90,
          "difficulty": "CLASS_400",
          "question": "وش اسم المدرب السعودي الأكثر نجاحاً في تاريخ الدوري السعودي للمحترفين؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نور الدين نايبت",
          "hint": "مدرب جزائري قاد الهلال لعدة بطولات",
          "points": 400
        },
        {
          "id": 91,
          "difficulty": "CLASS_200",
          "question": "على أي نهر أو وادٍ تقع محافظة الزلفي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "وادي الزلفي",
          "hint": "يمتد من الشمال إلى الجنوب عبر المحافظة",
          "points": 200
        },
        {
          "id": 92,
          "difficulty": "CLASS_400",
          "question": "وش اسم المحافظة المجاورة للزلفي من الجنوب؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "المجمعة",
          "hint": "مركز محافظة المجمعة المجاورة",
          "points": 400
        },
        {
          "id": 93,
          "difficulty": "CLASS_600",
          "question": "وش أصل كلمة 'الزلفي' وما معناها التاريخي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "تعني القُربى أو المنزلة (الاقتراب)",
          "hint": "من الجذر اللغوي 'زلف' بمعنى الاقتراب",
          "points": 600
        },
        {
          "id": 94,
          "difficulty": "CLASS_400",
          "question": "وش اسم أبرز مهرجان سنوي يُقام في محافظة الزلفي يحتفي بالتمر؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1605027990121-cbae9e0642df?w=640&q=80",
          "correctAnswer": "مهرجان تمور الزلفي",
          "hint": "يُقام في موسم حصاد التمر",
          "points": 400
        },
        {
          "id": 95,
          "difficulty": "CLASS_600",
          "question": "وش اسم الشاعر الشعبي المشهور من محافظة الزلفي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ابن شبيب (من أعلام الزلفي الشعرية)",
          "hint": "من أبرز شعراء النبط في المنطقة",
          "points": 600
        },
        {
          "id": 96,
          "difficulty": "CLASS_200",
          "question": "وش اسم الطريق الرئيسي الذي يمر بمحافظة الزلفي ويربطها بالرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "طريق القصيم السريع (طريق رقم 65)",
          "hint": "الطريق الرابط بين الرياض والقصيم مروراً بالزلفي",
          "points": 200
        },
        {
          "id": 97,
          "difficulty": "CLASS_400",
          "question": "وش اسم شركة السيارات الأمريكية اللي تصنع سيارة 'موستانج' و'F-150'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/ford.com",
          "correctAnswer": "فورد",
          "hint": "من أقدم شركات السيارات في أمريكا",
          "points": 400
        },
        {
          "id": 98,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة السيارات الكورية اللي تصنع 'سوناتا' و'توسان'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/hyundai.com",
          "correctAnswer": "هيونداي",
          "hint": "شعارها حرف H مائل",
          "points": 200
        },
        {
          "id": 99,
          "difficulty": "CLASS_400",
          "question": "وش اسم سيارة لاند روفر الفاخرة الأشهر والأغلى ثمناً؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/landrover.com",
          "correctAnswer": "رينج روفر",
          "hint": "سيارة دفع رباعي فاخرة بريطانية",
          "points": 400
        },
        {
          "id": 100,
          "difficulty": "CLASS_600",
          "question": "وش اسم سيارة بنتلي الرياضية الفاخرة الأعلى أداءً؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/bentleymotors.com",
          "correctAnswer": "بنتلي كونتيننتال GT",
          "hint": "سيارة غران توريزمو فاخرة بريطانية",
          "points": 600
        },
        {
          "id": 101,
          "difficulty": "CLASS_200",
          "question": "وش اسم سلسلة مطاعم الدجاج الأمريكية اللي شعارها رجل عجوز بربطة عنق حمراء؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/kfc.com",
          "correctAnswer": "كنتاكي - KFC",
          "hint": "الاسم اختصار لـ Kentucky Fried Chicken",
          "points": 200
        },
        {
          "id": 102,
          "difficulty": "CLASS_200",
          "question": "وش اسم سلسلة قهوة أمريكية شهيرة جداً شعارها حورية البحر؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=640&q=80",
          "correctAnswer": "ستاربكس",
          "hint": "من أكثر سلاسل القهوة انتشاراً في العالم",
          "points": 200
        },
        {
          "id": 103,
          "difficulty": "CLASS_400",
          "question": "وش اسم المطعم السعودي المحلي المشهور بالسندويشات والفراخ في الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "هرفي",
          "hint": "سلسلة سعودية شعبية بأسعار معقولة",
          "points": 400
        },
        {
          "id": 104,
          "difficulty": "CLASS_400",
          "question": "وش اسم سلسلة مطاعم البيتزا الأمريكية اللي شعارها سقف أحمر وشخص يحمل البيتزا؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/pizzahut.com",
          "correctAnswer": "بيتزا هت",
          "hint": "من أوائل سلاسل البيتزا في العالم",
          "points": 400
        },
        {
          "id": 105,
          "difficulty": "CLASS_600",
          "question": "وش اسم سلسلة المطاعم السعودية الشهيرة بالكبدة والطعام الشعبي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مطعم أبو زيد",
          "hint": "مطعم شعبي سعودي للمأكولات النجدية",
          "points": 600
        },
        {
          "id": 106,
          "difficulty": "CLASS_200",
          "question": "وش معنى كلمة 'حاشي' باللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الجمل الصغير",
          "hint": "يقال في السياق البدوي للدلالة على صغير الجمال",
          "points": 200
        },
        {
          "id": 107,
          "difficulty": "CLASS_200",
          "question": "وش معنى كلمة 'سالفة' باللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "قصة أو حديث أو موضوع",
          "hint": "تقوله لما تحكي لأحد حدثاً أو خبراً",
          "points": 200
        },
        {
          "id": 108,
          "difficulty": "CLASS_400",
          "question": "وش معنى كلمة 'مستحيل' أو 'ما يصير' في الرد السعودي الرافض باللهجة الدارجة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "لا يمكن أو رفض قاطع",
          "hint": "'ما يصير هذا الشي' = هذا الأمر غير ممكن",
          "points": 400
        },
        {
          "id": 109,
          "difficulty": "CLASS_200",
          "question": "وش معنى 'طبّق' باللهجة السعودية الشبابية الحديثة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نفّذ الأمر أو افعله الآن",
          "hint": "يُستخدم للحثّ على التنفيذ الفوري",
          "points": 200
        },
        {
          "id": 110,
          "difficulty": "CLASS_600",
          "question": "وش معنى كلمة 'دوشة' في اللهجة السعودية الحجازية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ضوضاء أو فوضى أو إزعاج",
          "hint": "تقولها لما يكون في ضجيج أو فوضى",
          "points": 600
        },
        {
          "id": 111,
          "difficulty": "CLASS_200",
          "question": "وش معنى كلمة 'عدل' في اللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "صحيح أو مضبوط أو موافق",
          "hint": "'عدل كلامك' تعني كلامك صحيح",
          "points": 200
        },
        {
          "id": 112,
          "difficulty": "CLASS_400",
          "question": "وش معنى 'ما عنده خلق' في اللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ليس لديه طاقة أو رغبة أو همّة",
          "hint": "تقولها لما تكون كسلان وما تبي تتحرك",
          "points": 400
        },
        {
          "id": 113,
          "difficulty": "CLASS_400",
          "question": "وش معنى كلمة 'شيله' في السياق السعودي التجاري؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "خذه أو أخذ البضاعة",
          "hint": "تُستخدم في السوق: 'شيله وامشي'",
          "points": 400
        },
        {
          "id": 114,
          "difficulty": "CLASS_200",
          "question": "وش معنى 'خوش' في اللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "جيد أو ممتاز",
          "hint": "'خوش كلام' = كلام جيد ومعقول",
          "points": 200
        },
        {
          "id": 115,
          "difficulty": "CLASS_600",
          "question": "وش معنى 'دبّست' في اللهجة السعودية الشعبية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "وضعت الشيء في مكانه أو غرزته",
          "hint": "تُستخدم عند وضع أو تثبيت شيء ما",
          "points": 600
        },
        {
          "id": 116,
          "difficulty": "CLASS_400",
          "question": "وش معنى كلمة 'حمار' في سياق اللهجة السعودية العامية المجازي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "شخص غير ذكي أو يتصرف بطريقة غير لائقة",
          "hint": "تُستخدم مجازاً للتعبير عن الغباء أو الحماقة",
          "points": 400
        },
        {
          "id": 117,
          "difficulty": "CLASS_200",
          "question": "وش معنى 'بكره' في اللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "غداً أو في المستقبل القريب",
          "hint": "'نشوفك بكره' = نراك غداً",
          "points": 200
        },
        {
          "id": 118,
          "difficulty": "CLASS_400",
          "question": "وش معنى 'الطلعة' في اللهجة الشبابية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الخروج مع الأصدقاء أو نزهة غير رسمية",
          "hint": "'نسوّي طلعة الليلة' = نخرج مع بعض",
          "points": 400
        },
        {
          "id": 119,
          "difficulty": "CLASS_600",
          "question": "وش معنى 'مكلّح' في اللهجة الحجازية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مزيّن بشكل مبالغ أو لابس بطريقة ملفتة",
          "hint": "تُستخدم للشخص اللي يلبس بطريقة مبالغ فيها",
          "points": 600
        },
        {
          "id": 120,
          "difficulty": "CLASS_200",
          "question": "وش معنى 'يلّا' في اللهجة السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "هيّا أو لنذهب أو حثّ على الفعل",
          "hint": "تُقال للتشجيع على التحرك أو البدء",
          "points": 200
        },
        {
          "id": 121,
          "difficulty": "CLASS_200",
          "question": "وش اسم الشركة الألمانية صاحبة سيارة 'جولف' و'باسات'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vw.com",
          "correctAnswer": "فولكس واجن (Volkswagen)",
          "hint": "شعارها حرفان V وW متداخلان",
          "points": 200
        },
        {
          "id": 122,
          "difficulty": "CLASS_400",
          "question": "وش اسم سيارة لكزس الرياضية الفاخرة المعروفة في السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/lexus.com",
          "correctAnswer": "لكزس LX",
          "hint": "من أشهر سيارات الدفع الرباعي الفاخرة",
          "points": 400
        },
        {
          "id": 123,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة السيارات الكورية اللي تصنع 'سبورتاج' و'سيراتو'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/kia.com",
          "correctAnswer": "كيا",
          "hint": "شعارها بيضاوي بحروف إنجليزية مائلة",
          "points": 200
        },
        {
          "id": 124,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول سيارة صُنعت في العالم؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mercedes-benz.com",
          "correctAnswer": "بنز باتنت موتورواجن 1886",
          "hint": "صنعها كارل بنز في ألمانيا وسُجّل براءة الاختراع في يناير 1886",
          "points": 600
        },
        {
          "id": 125,
          "difficulty": "CLASS_400",
          "question": "وش اسم الشركة الصانعة لسيارة 'تيسلا' الكهربائية ومن هو مؤسسها الأشهر؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/tesla.com",
          "correctAnswer": "تيسلا - إيلون ماسك (من أبرز مؤسسيها)",
          "hint": "أكثر شركة سيارات كهربائية قيمة في العالم",
          "points": 400
        },
        {
          "id": 126,
          "difficulty": "CLASS_200",
          "question": "وش اسم سلسلة المطاعم السعودية الشهيرة بالشاورما واللي فروعها في كل مكان؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "البيك",
          "hint": "سلسلة سعودية أصيلة بأسعار شعبية",
          "points": 200
        },
        {
          "id": 127,
          "difficulty": "CLASS_400",
          "question": "وش اسم مطعم البرغر الأمريكي اللي شعاره تاج ويتنافس مع ماكدونالدز؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "برغر كنج",
          "hint": "'Have it Your Way' شعاره الشهير",
          "points": 400
        },
        {
          "id": 128,
          "difficulty": "CLASS_200",
          "question": "وش اسم سلسلة قهوة سعودية محلية ناجحة جداً منافسة لستاربكس؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "القهوة السعودية (SFC) أو كافيه حجازي",
          "hint": "سلسلة قهوة سعودية بنكهة محلية",
          "points": 200
        },
        {
          "id": 129,
          "difficulty": "CLASS_600",
          "question": "وش اسم السلسلة السعودية المتخصصة في المخبوزات والحلويات والمعروفة بـ 'الراشد'؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مخبز الراشد",
          "hint": "من أقدم المخابز السعودية الشعبية",
          "points": 600
        },
        {
          "id": 130,
          "difficulty": "CLASS_200",
          "question": "وش اسم المطعم الأمريكي الشهير بالدجاج المقرمش وشعاره دلو أحمر؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/kfc.com",
          "correctAnswer": "بوبايز أو KFC",
          "hint": "سلسلة دجاج مقرمش أمريكية شهيرة",
          "points": 200
        },
        {
          "id": 131,
          "difficulty": "CLASS_200",
          "question": "وش اسم القناة السعودية الترفيهية الأشهر للشباب وتُبثّ من الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mbc.net",
          "correctAnswer": "MBC",
          "hint": "قناة تلفزيونية سعودية عربية خاصة",
          "points": 200
        },
        {
          "id": 132,
          "difficulty": "CLASS_400",
          "question": "وش اسم المطرب السعودي اللي يُلقَّب بـ\"فنان العرب\" وغنّى \"ودّي أشوفك\"؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "محمد عبده",
          "hint": "من أيقونات الطرب الخليجي والعربي",
          "points": 400
        },
        {
          "id": 133,
          "difficulty": "CLASS_200",
          "question": "وش اسم المطرب السعودي الشهير اللي يُلقّب بـ'فنان العرب'؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "محمد عبده",
          "hint": "من أيقونات الطرب العربي السعودي",
          "points": 200
        },
        {
          "id": 134,
          "difficulty": "CLASS_400",
          "question": "وش اسم الفنانة السعودية الأولى اللي غنّت في الخارج ونالت جوائز دولية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "رابعة (أو نوال الكويتية - من المنطقة الخليجية)",
          "hint": "من الرائدات في غناء المرأة الخليجية",
          "points": 400
        },
        {
          "id": 135,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول حفلة موسيقية مختلطة رسمية في المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "حفلة مارك رونسون في جدة 2018م",
          "hint": "أُقيمت في إطار إصلاحات رؤية 2030",
          "points": 600
        },
        {
          "id": 136,
          "difficulty": "CLASS_200",
          "question": "وش اسم العصير الشعبي السعودي المصنوع من الليمون مع النعناع والمنتشر في كل مطعم؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "عصير الليمون بالنعناع",
          "hint": "مشروب رمضاني وصيفي خفيف",
          "points": 200
        },
        {
          "id": 137,
          "difficulty": "CLASS_400",
          "question": "وش اسم الحلوى السعودية الشعبية المصنوعة من الدبس والسمسم؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "السمسمية أو الدبسة",
          "hint": "حلوى تراثية شعبية قديمة",
          "points": 400
        },
        {
          "id": 138,
          "difficulty": "CLASS_200",
          "question": "وش اسم الطبق السعودي المشهور المصنوع من الخبز المفتّت مع اللحم والمرق؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "المرقوق (أو الجريش)",
          "hint": "من أشهر أطباق المطبخ النجدي التراثي",
          "points": 200
        },
        {
          "id": 139,
          "difficulty": "CLASS_400",
          "question": "وش اسم الفطيرة السعودية المصنوعة من الدقيق والسمن والعسل والمشهورة في الجنوب؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الرغيف أو المطازيز",
          "hint": "فطيرة تراثية من منطقة عسير وجازان",
          "points": 400
        },
        {
          "id": 140,
          "difficulty": "CLASS_200",
          "question": "وش اسم الفاكهة اللي تُعتبر أيقونة المملكة وتُصدَّر لكل العالم من أصناف مميزة؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1605027990121-cbae9e0642df?w=640&q=80",
          "correctAnswer": "التمر",
          "hint": "المملكة من أكبر منتجي التمور في العالم",
          "points": 200
        },
        {
          "id": 141,
          "difficulty": "CLASS_200",
          "question": "وش اسم الجبل الأشهر في مكة المكرمة واللي فيه الغار اللي نزل فيه أول وحي على النبي محمد ﷺ؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1564769625905-50e93615e769?w=640&q=80",
          "correctAnswer": "جبل النور - وفيه غار حراء",
          "hint": "فيه غار حراء حيث نزل الوحي لأول مرة",
          "points": 200
        },
        {
          "id": 142,
          "difficulty": "CLASS_400",
          "question": "وش اسم بحيرة تبوك الأثرية واللي تعدّ واحة تاريخية في الشمال؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "واحة تيماء",
          "hint": "من أقدم المناطق المأهولة في شمال الجزيرة العربية",
          "points": 400
        },
        {
          "id": 143,
          "difficulty": "CLASS_400",
          "question": "وش اسم الجزيرة السعودية السياحية الجميلة في البحر الأحمر المشهورة بالشعاب المرجانية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "جزر فرسان",
          "hint": "في منطقة جازان جنوب المملكة",
          "points": 400
        },
        {
          "id": 144,
          "difficulty": "CLASS_200",
          "question": "وش اسم المنطقة السياحية الشهيرة في أبها المعروفة بالضباب والخضرة؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=640&q=80",
          "correctAnswer": "منطقة عسير",
          "hint": "جبال عسير وأبها من أجمل مناطق المملكة",
          "points": 200
        },
        {
          "id": 145,
          "difficulty": "CLASS_600",
          "question": "وش اسم أعلى قمة جبلية في المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=640&q=80",
          "correctAnswer": "جبل السودة (3015 متر)",
          "hint": "يقع في منطقة عسير ويُعدّ أعلى جبل في المملكة",
          "points": 600
        },
        {
          "id": 146,
          "difficulty": "CLASS_400",
          "question": "وش اسم البحر اللي تطلّ عليه مدينة جدة؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1578895102073-a1481ebde81b?w=640&q=80",
          "correctAnswer": "البحر الأحمر",
          "hint": "يفصل المملكة عن القارة الأفريقية",
          "points": 400
        },
        {
          "id": 147,
          "difficulty": "CLASS_200",
          "question": "وش اسم الخليج العربي اللي تطلّ عليه المنطقة الشرقية في السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الخليج العربي (الخليج الفارسي)",
          "hint": "يقع شرق المملكة ويحدّها من الشرق",
          "points": 200
        },
        {
          "id": 148,
          "difficulty": "CLASS_600",
          "question": "وش اسم المنطقة الجغرافية الصحراوية الكبيرة التي تحتل معظم وسط المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الربع الخالي",
          "hint": "أكبر صحراء رملية متواصلة في العالم",
          "points": 600
        },
        {
          "id": 149,
          "difficulty": "CLASS_200",
          "question": "وش اسم المدينة السعودية الثانية من حيث الحجم بعد الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1578895102073-a1481ebde81b?w=640&q=80",
          "correctAnswer": "جدة",
          "hint": "عاصمة الحجاز وعروس البحر الأحمر",
          "points": 200
        },
        {
          "id": 150,
          "difficulty": "CLASS_400",
          "question": "وش اسم المنطقة الشمالية في المملكة التي تشتهر بزراعة القمح والزيتون؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "منطقة تبوك",
          "hint": "تقع شمال غرب المملكة",
          "points": 400
        },
        {
          "id": 151,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة السيارات الإيطالية اللي تصنع 'لامبورغيني' وشعارها ثور؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/lamborghini.com",
          "correctAnswer": "لامبورغيني",
          "hint": "سيارات سوبر كار إيطالية فاخرة جداً",
          "points": 200
        },
        {
          "id": 152,
          "difficulty": "CLASS_400",
          "question": "وش اسم سيارة BMW الرياضية الأشهر والأغلى؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/bmw.com",
          "correctAnswer": "BMW M5 أو BMW 7 Series",
          "hint": "من أفخر سيارات الشركة الألمانية",
          "points": 400
        },
        {
          "id": 153,
          "difficulty": "CLASS_600",
          "question": "وش اسم شركة السيارات اليابانية التي تصنع 'جي تي آر' المعروفة بـ Godzilla؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/nissan.com",
          "correctAnswer": "نيسان",
          "hint": "نيسان GTR من أسرع سيارات الإنتاج الجماعي",
          "points": 600
        },
        {
          "id": 154,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة السيارات الفرنسية اللي تصنع 'ميغان' و'كليو'؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/renault.com",
          "correctAnswer": "رينو",
          "hint": "شعارها ماسة صفراء",
          "points": 200
        },
        {
          "id": 155,
          "difficulty": "CLASS_400",
          "question": "وش اسم شركة السيارات الأمريكية اللي تصنع كورفيت وكامارو؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/chevrolet.com",
          "correctAnswer": "شيفروليه",
          "hint": "شعارها ربطة عنق ذهبية",
          "points": 400
        },
        {
          "id": 156,
          "difficulty": "CLASS_200",
          "question": "وش اسم تطبيق توصيل الطعام السعودي الأشهر باللون البرتقالي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "هنقرستيشن أو طلبات",
          "hint": "تطبيقات شهيرة لتوصيل الطعام في السعودية",
          "points": 200
        },
        {
          "id": 157,
          "difficulty": "CLASS_400",
          "question": "وش اسم شركة الشحن السعودية الحكومية الأشهر؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "البريد السعودي - سبل",
          "hint": "تغيّر اسمها من البريد السعودي لـ'سبل'",
          "points": 400
        },
        {
          "id": 158,
          "difficulty": "CLASS_200",
          "question": "وش اسم منصة التواصل الاجتماعي اللي يستخدمها السعوديون للاطلاع على آخر الأخبار الآن أكثر من غيرها؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/x.com",
          "correctAnswer": "تويتر (إكس)",
          "hint": "غيّر اسمه من تويتر بعد شراء ماسك له",
          "points": 200
        },
        {
          "id": 159,
          "difficulty": "CLASS_400",
          "question": "وش اسم منصة الفيديو القصير الصيني المنتشر بقوة بين الشباب السعودي؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/tiktok.com",
          "correctAnswer": "تيك توك",
          "hint": "منصة الفيديوهات القصيرة الشهيرة",
          "points": 400
        },
        {
          "id": 160,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول صحيفة يومية سعودية أُسِّست؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "صحيفة أم القرى (1924م)",
          "hint": "أُسّست في عهد الملك عبدالعزيز",
          "points": 600
        },
        {
          "id": 161,
          "difficulty": "CLASS_200",
          "question": "كم عدد مناطق المملكة العربية السعودية الإدارية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "13 منطقة إدارية",
          "hint": "تشمل: الرياض، مكة، المدينة، وغيرها",
          "points": 200
        },
        {
          "id": 162,
          "difficulty": "CLASS_400",
          "question": "وش اسم أكبر منطقة جغرافياً في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "منطقة الرياض",
          "hint": "تحتل وسط المملكة وهي الأوسع مساحة",
          "points": 400
        },
        {
          "id": 163,
          "difficulty": "CLASS_400",
          "question": "وش اسم العملة الرسمية للمملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الريال السعودي",
          "hint": "يُرمز له بـ ر.س",
          "points": 400
        },
        {
          "id": 164,
          "difficulty": "CLASS_200",
          "question": "وش اسم أكبر مطار في المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مطار الملك عبدالعزيز الدولي في جدة",
          "hint": "يستقبل الحجاج والمعتمرين من كل العالم",
          "points": 200
        },
        {
          "id": 165,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول سفير سعودي في الأمم المتحدة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الأمير فيصل بن عبدالعزيز (كان وزيراً للخارجية)",
          "hint": "وزير الخارجية الذي قاد وفد المملكة",
          "points": 600
        },
        {
          "id": 166,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة الخطوط الجوية الوطنية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/saudia.com",
          "correctAnswer": "الخطوط السعودية",
          "hint": "شعارها نخلة ومعروفة بـ Saudia",
          "points": 200
        },
        {
          "id": 167,
          "difficulty": "CLASS_400",
          "question": "وش اسم الناقي السعودي الذي يُقدّم برنامج 'إكس فاكتور العرب' وهو من جدة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "عمر خيرت أو راشد الماجد",
          "hint": "من مشاهير الفن السعودي الحجازي",
          "points": 400
        },
        {
          "id": 168,
          "difficulty": "CLASS_400",
          "question": "وش اسم أشهر سوق شعبي في مدينة الرياض وهو من أشهر المعالم التراثية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "سوق الزل أو سوق الديرة (سوق الخرج القديم)",
          "hint": "من أعرق الأسواق الشعبية في العاصمة",
          "points": 400
        },
        {
          "id": 169,
          "difficulty": "CLASS_200",
          "question": "وش اسم أشهر مركز تجاري في الرياض؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مركز الرياض بارك أو العثيم مول أو موبايل بارك",
          "hint": "مراكز تسوق كبرى في العاصمة",
          "points": 200
        },
        {
          "id": 170,
          "difficulty": "CLASS_400",
          "question": "وش اسم المنطقة الترفيهية الجديدة في الرياض المخصصة للفعاليات الكبرى؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "السعودية أرينا أو البوليفارد رياض سيتي",
          "hint": "من أبرز مشاريع الترفيه في العاصمة",
          "points": 400
        },
        {
          "id": 171,
          "difficulty": "CLASS_200",
          "question": "وش اسم الحيوان الوطني للمملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الصقر",
          "hint": "يُمثّل القوة والحرية في الثقافة العربية",
          "points": 200
        },
        {
          "id": 172,
          "difficulty": "CLASS_400",
          "question": "وش اسم الشجرة الوطنية للمملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://images.unsplash.com/photo-1605027990121-cbae9e0642df?w=640&q=80",
          "correctAnswer": "النخلة",
          "hint": "رمز الخير والنماء في الثقافة العربية",
          "points": 400
        },
        {
          "id": 173,
          "difficulty": "CLASS_600",
          "question": "وش اسم أكبر جامعة حكومية في المملكة من حيث عدد الطلاب؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "جامعة الملك سعود في الرياض",
          "hint": "أسسها الملك سعود بن عبدالعزيز عام 1957م",
          "points": 600
        },
        {
          "id": 174,
          "difficulty": "CLASS_200",
          "question": "وش اسم الحرب اللي شاركت فيها قوات التحالف بقيادة السعودية في اليمن منذ 2015؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "عملية عاصفة الحزم",
          "hint": "بدأت في مارس 2015م",
          "points": 200
        },
        {
          "id": 175,
          "difficulty": "CLASS_400",
          "question": "وش اسم منظمة الدول المصدّرة للنفط التي تنتمي إليها المملكة العربية السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/aramco.com",
          "correctAnswer": "أوبك (OPEC)",
          "hint": "منظمة الدول المصدّرة للبترول",
          "points": 400
        },
        {
          "id": 176,
          "difficulty": "CLASS_200",
          "question": "وش اسم شركة الطيران السعودية الجديدة المنافسة للخطوط السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "طيران ناس أو فلاي ناس",
          "hint": "شركة طيران سعودية منخفضة التكلفة",
          "points": 200
        },
        {
          "id": 177,
          "difficulty": "CLASS_400",
          "question": "وش اسم المشروع الضخم اللي سيجعل الرياض من أكبر مدن العالم في 2030؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "مشروع مطلع أو مشروع نيوم أو تطوير الرياض",
          "hint": "ضمن رؤية 2030 لتوسيع العاصمة",
          "points": 400
        },
        {
          "id": 178,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول امرأة سعودية تُعيَّن سفيرة في الخارج؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "الأميرة ريما بنت بندر (سفيرة لدى الولايات المتحدة)",
          "hint": "أول سفيرة سعودية في تاريخ المملكة",
          "points": 600
        },
        {
          "id": 179,
          "difficulty": "CLASS_200",
          "question": "وش اسم أول نادٍ ترفيهي رسمي افتُتح في المملكة بعد رفع الحظر؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "Cinema AMC (أول سينما رسمية في الرياض 2018)",
          "hint": "في مركز المملكة بالرياض",
          "points": 200
        },
        {
          "id": 180,
          "difficulty": "CLASS_400",
          "question": "وش اسم الفعالية الرياضية الكبرى اللي استضافتها المملكة عام 2023 في الغولف؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "بطولة LIV Golf",
          "hint": "دوري الغولف الجديد الذي أسسه صندوق الاستثمارات العامة",
          "points": 400
        },
        {
          "id": 181,
          "difficulty": "CLASS_200",
          "question": "وش اسم الرياضة اللي فازت فيها السعودية ببطولة عالمية وأقامت احتفالاً ضخماً؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "كرة القدم (الفوز على الأرجنتين في كأس العالم 2022)",
          "hint": "مباراة تاريخية لا تُنسى",
          "points": 200
        },
        {
          "id": 182,
          "difficulty": "CLASS_400",
          "question": "وش اسم النادي الرياضي السعودي الذي انضم إليه نيمار البرازيلي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "نادي الهلال",
          "hint": "النادي الأزرق الملكي",
          "points": 400
        },
        {
          "id": 183,
          "difficulty": "CLASS_600",
          "question": "وش اسم اللاعب السعودي الأول في تاريخ كرة القدم الذي لعب في الدوري الألماني؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "سامي الجابر - لعب لفولفسبورغ الألماني",
          "hint": "أسطورة الهلال لعب في الدوري الألماني عام 1997",
          "points": 600
        },
        {
          "id": 184,
          "difficulty": "CLASS_200",
          "question": "وش اسم الحفل الرياضي الكبير اللي تستضيفه المملكة لأول مرة عام 2034؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "كأس العالم FIFA 2034",
          "hint": "المملكة فازت باستضافة أكبر حدث رياضي في العالم",
          "points": 200
        },
        {
          "id": 185,
          "difficulty": "CLASS_400",
          "question": "وش اسم الحدث العالمي الكبير اللي تستضيفه المملكة عام 2030؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "معرض إكسبو 2030 في الرياض",
          "hint": "المعرض العالمي الأشمل للابتكار والتكنولوجيا",
          "points": 400
        },
        {
          "id": 186,
          "difficulty": "CLASS_200",
          "question": "وش اسم رياضة الفروسية السعودية الشعبية التراثية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "المطية أو التحطيب",
          "hint": "رياضة فنون القتال التراثية بالعصا",
          "points": 200
        },
        {
          "id": 187,
          "difficulty": "CLASS_400",
          "question": "وش اسم الشركة السعودية الأكثر قيمة بعد أرامكو في سوق الأسهم السعودي؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/stc.com.sa",
          "correctAnswer": "شركة الاتصالات السعودية (STC)",
          "hint": "من أكبر شركات الاتصالات إقليمياً",
          "points": 400
        },
        {
          "id": 188,
          "difficulty": "CLASS_600",
          "question": "وش اسم سوق الأسهم السعودية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "تداول (سوق الأسهم السعودية)",
          "hint": "من أكبر الأسواق المالية في منطقة الشرق الأوسط",
          "points": 600
        },
        {
          "id": 189,
          "difficulty": "CLASS_400",
          "question": "وش اسم صندوق الاستثمارات الحكومي السعودي الأكبر؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/vision2030.gov.sa",
          "correctAnswer": "صندوق الاستثمارات العامة (PIF)",
          "hint": "من أكبر صناديق الثروة السيادية في العالم",
          "points": 400
        },
        {
          "id": 190,
          "difficulty": "CLASS_200",
          "question": "وش اسم الجهة الحكومية السعودية المسؤولة عن الإفتاء والشريعة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "هيئة كبار العلماء",
          "hint": "أعلى مرجعية دينية في المملكة",
          "points": 200
        },
        {
          "id": 191,
          "difficulty": "CLASS_400",
          "question": "وش اسم المشروع السعودي الأضخم اللي يُعدّ خطاً معمارياً مستقيماً طوله 170 كيلومتر؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/neom.com",
          "correctAnswer": "ذا لاين (The Line)",
          "hint": "مدينة بلا سيارات تعمل بالطاقة المتجددة",
          "points": 400
        },
        {
          "id": 192,
          "difficulty": "CLASS_200",
          "question": "وش اسم المشاهير اليوتيوب السعودي الذي يُعدّ من أكثر المحتوى العربي مشاهدةً؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "عمر يوسف أو بدر صالح (من أشهر اليوتيوبرز السعوديين)",
          "hint": "صنّاع محتوى كوميدي وترفيهي سعودي",
          "points": 200
        },
        {
          "id": 193,
          "difficulty": "CLASS_400",
          "question": "وش اسم أشهر شاعر نبطي سعودي معاصر مشهور في قصائد الحنين والوطنية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "بدر بن عبدالمحسن",
          "hint": "أمير وشاعر سعودي كبير رحمه الله",
          "points": 400
        },
        {
          "id": 194,
          "difficulty": "CLASS_200",
          "question": "وش اسم اليوم الوطني للمملكة العربية السعودية ومتى يُحتفل فيه؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "اليوم الوطني السعودي في 23 سبتمبر",
          "hint": "يُحيي توحيد المملكة عام 1932م",
          "points": 200
        },
        {
          "id": 195,
          "difficulty": "CLASS_400",
          "question": "وش اسم الجهة الحكومية اللي تُشرف على الإعلام في المملكة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "وزارة الإعلام السعودية",
          "hint": "تُنظّم القطاع الإعلامي والصحفي",
          "points": 400
        },
        {
          "id": 196,
          "difficulty": "CLASS_200",
          "question": "وش اسم برنامج مسابقات الغناء العربي الأشهر اللي بدأ من قناة MBC؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/mbc.net",
          "correctAnswer": "Arab Idol",
          "hint": "نسخة عربية من برنامج American Idol",
          "points": 200
        },
        {
          "id": 197,
          "difficulty": "CLASS_600",
          "question": "وش اسم أول فيلم سعودي يُعرض في مهرجان فينيسيا السينمائي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "فيلم وجدة للمخرجة هيفاء المنصور",
          "hint": "أول فيلم طويل تُخرجه امرأة سعودية",
          "points": 600
        },
        {
          "id": 198,
          "difficulty": "CLASS_400",
          "question": "وش اسم أول لاعبة سعودية تشارك في الألعاب الأولمبية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "وجدان شهركاني (أولمبياد لندن 2012)",
          "hint": "شاركت في الجودو وكانت تاريخية",
          "points": 400
        },
        {
          "id": 199,
          "difficulty": "CLASS_200",
          "question": "وش اسم اللاعب السعودي الأسطورة الذي كان يلعب للهلال ويُلقّب بـ'عميد الكرة السعودية'؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "ماجد عبدالله",
          "hint": "أيقونة كرة القدم السعودية الأشهر",
          "points": 200
        },
        {
          "id": 200,
          "difficulty": "CLASS_600",
          "question": "وش اسم الاتفاقية التاريخية اللي أبرمتها السعودية مع أمريكا عام 1945 على متن سفينة يو إس إس كوينسي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "اتفاقية الملك عبدالعزيز والرئيس روزفلت (النفط مقابل الأمن)",
          "hint": "أسّست لشراكة استراتيجية طويلة المدى بين البلدين",
          "points": 600
        },
        {
          "id": 201,
          "difficulty": "CLASS_200",
          "question": "كم عدد قنوات شبكة المجد التلفزيونية تقريباً؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "أكثر من 10 قنوات متخصصة",
          "hint": "تشمل قرآن وعلمية وأطفال وتاريخ وأسرة",
          "points": 400
        },
        {
          "id": 202,
          "difficulty": "CLASS_400",
          "question": "وش اسم القناة الإسلامية في شبكة المجد المخصصة للمرأة المسلمة والأسرة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مجد الاجتماعية",
          "hint": "تعنى بقضايا المرأة والأسرة المسلمة",
          "points": 400
        },
        {
          "id": 203,
          "difficulty": "CLASS_200",
          "question": "وش اسم برنامج شبكة المجد الذي يُعرض تلاوات القرآن الكريم المستمرة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "بث مجد القرآن الكريم",
          "hint": "يعرض تلاوات للقراء المتميزين",
          "points": 200
        },
        {
          "id": 204,
          "difficulty": "CLASS_400",
          "question": "وش اسم البرنامج الذي تقدمه شبكة المجد لتعليم قراءة القرآن الكريم للأطفال؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "برامج قناة مجد للأطفال التعليمية",
          "hint": "تضم دروساً لتحفيظ القرآن",
          "points": 400
        },
        {
          "id": 205,
          "difficulty": "CLASS_600",
          "question": "في أي مدينة يقع مقر شبكة المجد التلفزيونية الرئيسي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "الرياض - المملكة العربية السعودية",
          "hint": "عاصمة المملكة",
          "points": 600
        },
        {
          "id": 206,
          "difficulty": "CLASS_400",
          "question": "وش اسم البرنامج الديني على شبكة المجد الذي يُعقد فيه نقاشات علمية بين علماء الشريعة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مجلس العلماء أو فقه الحياة",
          "hint": "حلقات علمية يشارك فيها أكثر من عالم",
          "points": 400
        },
        {
          "id": 207,
          "difficulty": "CLASS_200",
          "question": "وش اسم قناة شبكة المجد المخصصة لعرض البرامج الوثائقية والعلمية الإسلامية؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "مجد العلمية",
          "hint": "وثائقيات علمية بنظرة إسلامية",
          "points": 200
        },
        {
          "id": 208,
          "difficulty": "CLASS_400",
          "question": "وش اسم تطبيق شبكة المجد الذي يمكن من خلاله متابعة قنواتها على الجوال؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "تطبيق مجد",
          "hint": "متاح على iOS وAndroid",
          "points": 400
        },
        {
          "id": 209,
          "difficulty": "CLASS_200",
          "question": "وش اسم الحاسب الصغير الذي طورته شركة آبل ويعمل بنظام macOS؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/apple.com",
          "correctAnswer": "ماك بوك",
          "hint": "MacBook — لابتوب آبل الشهير",
          "points": 200
        },
        {
          "id": 210,
          "difficulty": "CLASS_200",
          "question": "وش اسم نظام التشغيل المستخدم في هواتف شركة آبل؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/apple.com",
          "correctAnswer": "iOS",
          "hint": "يختلف عن Android",
          "points": 200
        },
        {
          "id": 211,
          "difficulty": "CLASS_400",
          "question": "وش اسم عملة تيثر الرقمية المشهورة والمثبّتة بالدولار الأمريكي؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "USDT أو تيثر",
          "hint": "من أشهر العملات الرقمية المستقرة",
          "points": 400
        },
        {
          "id": 212,
          "difficulty": "CLASS_200",
          "question": "وش اسم التطبيق الأزرق الشهير لمحادثات الفيديو والصوت؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/zoom.us",
          "correctAnswer": "زوم Zoom",
          "hint": "انتشر بشكل ضخم في جائحة كورونا",
          "points": 200
        },
        {
          "id": 213,
          "difficulty": "CLASS_400",
          "question": "وش اسم قاعدة البيانات الأكثر استخداماً في تطوير المواقع والتطبيقات؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "MySQL",
          "hint": "قاعدة بيانات علائقية مفتوحة المصدر",
          "points": 400
        },
        {
          "id": 214,
          "difficulty": "CLASS_600",
          "question": "وش اسم الذكاء الاصطناعي الشهير الذي طورته شركة OpenAI وانتشر عام 2022؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/openai.com",
          "correctAnswer": "ChatGPT",
          "hint": "نموذج لغوي ضخم قلب عالم التقنية",
          "points": 600
        },
        {
          "id": 215,
          "difficulty": "CLASS_200",
          "question": "وش اسم منصة التواصل الاحترافي للموظفين وأصحاب الأعمال؟",
          "mediaType": "image",
          "mediaUrl": "https://logo.clearbit.com/linkedin.com",
          "correctAnswer": "لينكدإن LinkedIn",
          "hint": "مخصص لعرض السيرة الذاتية والتواصل المهني",
          "points": 200
        },
        {
          "id": 216,
          "difficulty": "CLASS_400",
          "question": "وش اسم التقنية اللاسلكية قصيرة المدى المستخدمة في نقل الملفات بين الأجهزة القريبة؟",
          "mediaType": "image",
          "mediaUrl": "https://flagpedia.net/data/flags/w580/sa.webp",
          "correctAnswer": "بلوتوث Bluetooth",
          "hint": "رمزها شعار B أزرق",
          "points": 400
        }
      ]
    },
    "najdi_proverbs": {
      "name": "أمثال نجدية بالإيموجي",
      "description": "خمّن المثل النجدي من الإيموجيات",
      "totalQuestions": 50,
      "questions": [
        {
          "id": 1,
          "category": "أمثال نجدية",
          "emojis": "☕🤫",
          "proverb": "الدلة ما تكذب",
          "meaning": "القهوة (الدلة) تكشف طبيعة صاحب البيت وكرمه من أول جلسة — الأفعال أصدق من الكلام",
          "hint": "القهوة العربية تفضح صاحبها",
          "points": 200
        },
        {
          "id": 2,
          "category": "أمثال نجدية",
          "emojis": "🌴💧",
          "proverb": "النخلة ما تحمل إلا بالماء",
          "meaning": "ما في شيء يكبر أو ينجح بدون جهد وعناية — النتائج تحتاج أسباب",
          "hint": "أشهر شجرة في نجد وسرّ نموها",
          "points": 200
        },
        {
          "id": 3,
          "category": "أمثال نجدية",
          "emojis": "🐑🤲",
          "proverb": "اللي ما عنده غنم يرعى غنم الناس",
          "meaning": "اللي ما عنده مصلحة خاصة يشتغل لصالح غيره — يُقال للتبعي اللي ينفذ أوامر الناس",
          "hint": "الرعي والغنم من أساسيات الحياة النجدية",
          "points": 200
        },
        {
          "id": 4,
          "category": "أمثال نجدية",
          "emojis": "🗡️🌵",
          "proverb": "الكلمة اللي ما يردّها السيف تردّها الكلمة",
          "meaning": "الحكمة في الكلام قد تحلّ ما عجز عنه القوة — الحوار أقوى من المواجهة",
          "hint": "السيف والكلمة أداتان مختلفتان",
          "points": 400
        },
        {
          "id": 5,
          "category": "أمثال نجدية",
          "emojis": "🐫🛣️",
          "proverb": "الجمل ما يشوف سنامه",
          "meaning": "الإنسان لا يرى عيوب نفسه وإن كانت ظاهرة للجميع",
          "hint": "أبرز شيء في الجمل لكنه ما يشوفه",
          "points": 200
        },
        {
          "id": 6,
          "category": "أمثال نجدية",
          "emojis": "🏠🌧️",
          "proverb": "بيتك يكفيك وإن كان ضيق",
          "meaning": "القناعة بما عندك أفضل من طمع في بيت الغير — البيت الصغير برحابة أهله",
          "hint": "خير وكن في بيتك",
          "points": 200
        },
        {
          "id": 7,
          "category": "أمثال نجدية",
          "emojis": "👅🔪",
          "proverb": "اللسان ما له عظم لكنه يكسر العظم",
          "meaning": "الكلمة الجارحة أشد أثراً من السيف — الإيذاء بالقول أشد من الجسد",
          "hint": "الكلام يبني ويهدم",
          "points": 200
        },
        {
          "id": 8,
          "category": "أمثال نجدية",
          "emojis": "🪣💧🔚",
          "proverb": "من حفر بير لأخوه وقع فيه",
          "meaning": "من أراد الشر لغيره رجع عليه — الكيد يعود على صاحبه",
          "hint": "البئر والمكيدة",
          "points": 200
        },
        {
          "id": 9,
          "category": "أمثال نجدية",
          "emojis": "☀️🧤",
          "proverb": "الشمس ما تغطى بغربال",
          "meaning": "الحق والوضوح لا يمكن إخفاؤهما مهما حاولت — الحقيقة تظهر دائماً",
          "hint": "الغربال أداة مشقوقة لا تحجب",
          "points": 200
        },
        {
          "id": 10,
          "category": "أمثال نجدية",
          "emojis": "🐐🌵",
          "proverb": "العنز في عينها خير من الغزال",
          "meaning": "كل إنسان يرى ما عنده أحسن شيء — الحب يعمي العيون",
          "hint": "الأم وابنها",
          "points": 200
        },
        {
          "id": 11,
          "category": "أمثال نجدية",
          "emojis": "🧱🕰️",
          "proverb": "اللي يبني على أساس ما يخشى",
          "meaning": "من يبني حياته على أسس متينة من شغل وأخلاق ما يخاف المستقبل",
          "hint": "الأساس القوي يثبّت البناء",
          "points": 400
        },
        {
          "id": 12,
          "category": "أمثال نجدية",
          "emojis": "🐍🔴",
          "proverb": "اللي ينهشه الثعبان يخاف من الحبل",
          "meaning": "من جرّبه شيء سيء صار يخشى حتى ما يشبهه — التجربة المؤلمة تزرع الخوف",
          "hint": "لدغة تعلّم الحذر",
          "points": 400
        },
        {
          "id": 13,
          "category": "أمثال نجدية",
          "emojis": "🌾🤲",
          "proverb": "حصّادك من زرعت",
          "meaning": "ما تزرعه من أفعال وأخلاق هو ما ستجنيه — الجزاء من جنس العمل",
          "hint": "الزراعة والحصاد دورة لا تكذب",
          "points": 200
        },
        {
          "id": 14,
          "category": "أمثال نجدية",
          "emojis": "🪨🌊",
          "proverb": "الصبر مرّ لكن عاقبته حلوة",
          "meaning": "الصبر على الشدائد يصعب في بدايته لكن نتيجته خير وفرج",
          "hint": "الصبر طعمه معروف",
          "points": 200
        },
        {
          "id": 15,
          "category": "أمثال نجدية",
          "emojis": "🐺🐑",
          "proverb": "الذيب ما يتوب من الغنم",
          "meaning": "الشخص الفاسد لا تتوقع منه إصلاح — الطبع الغالب لا يتغير",
          "hint": "العدو الطبيعي للغنم",
          "points": 200
        },
        {
          "id": 16,
          "category": "أمثال نجدية",
          "emojis": "🪞👀",
          "proverb": "شوف عيبك قبل ما تشوف عيب الناس",
          "meaning": "ابدأ بنقد نفسك قبل أن تنتقد الآخرين — أصلح نفسك أولاً",
          "hint": "المرآة أداة المحاسبة",
          "points": 200
        },
        {
          "id": 17,
          "category": "أمثال نجدية",
          "emojis": "🏜️🚶",
          "proverb": "اللي يسير على الدرب يوصل",
          "meaning": "المثابر والصابر على طريقه يصل لهدفه في النهاية — الاستمرارية تحقق النتائج",
          "hint": "الخطوة الثابتة تقطع المسافة",
          "points": 200
        },
        {
          "id": 18,
          "category": "أمثال نجدية",
          "emojis": "🫙🍯",
          "proverb": "خذ من الدنيا ما صفّالك",
          "meaning": "اقبل ما تيسّر لك من الدنيا ولا تُرهق نفسك بما يكدّر — القناعة راحة",
          "hint": "الصافي يريح، والكدر يتعب",
          "points": 400
        },
        {
          "id": 19,
          "category": "أمثال نجدية",
          "emojis": "🧂🥘",
          "proverb": "الملح ما يفسد القدر",
          "meaning": "الناصح الأمين لا يضر — الكلمة الطيبة في وقتها تُصلح ولا تُفسد",
          "hint": "الملح يُصلح الطعام ولا يضره",
          "points": 400
        },
        {
          "id": 20,
          "category": "أمثال نجدية",
          "emojis": "🌵🌧️",
          "proverb": "الأرض البيضاء تبيّن فيها الأثر",
          "meaning": "الشخص النظيف السيرة يظهر فيه أي خطأ بوضوح — الطيب ينكشف فيه العيب سريعاً",
          "hint": "الأرض النظيفة تظهر عليها أصغر أثر",
          "points": 600
        },
        {
          "id": 21,
          "category": "أمثال نجدية",
          "emojis": "🤫🧏",
          "proverb": "اللي ما يعرف الصقر يشويه",
          "meaning": "الجاهل بقيمة الشيء النفيس يُهينه أو يُتلفه — الجهل يُهدر القيمة",
          "hint": "الصقر طير نفيس لكن الجاهل ما يعرف",
          "points": 400
        },
        {
          "id": 22,
          "category": "أمثال نجدية",
          "emojis": "☁️🌬️",
          "proverb": "كلام الليل يمحيه النهار",
          "meaning": "الوعود والكلام في جلسات السمر كثيراً ما يُنسى أو يُراجَع في وضح النهار",
          "hint": "الليل والنهار حالتان مختلفتان",
          "points": 400
        },
        {
          "id": 23,
          "category": "أمثال نجدية",
          "emojis": "🎯🌵",
          "proverb": "اللي يصيد العصافير ما يصيد الكباش",
          "meaning": "من اعتاد الأمور الصغيرة لا يُؤهَّل للأمور الكبيرة — كل إنسان مقياسه مختلف",
          "hint": "الصيد يختلف باختلاف القدرة والهمّة",
          "points": 600
        },
        {
          "id": 24,
          "category": "أمثال نجدية",
          "emojis": "🐪💨",
          "proverb": "جاء يكحّل عيونها عمّاها",
          "meaning": "جاء ليُصلح الأمر فأفسده أكثر — نيّة الإصلاح مع نتيجة التخريب",
          "hint": "الكحل يزيّن العين ولا يعميها",
          "points": 400
        },
        {
          "id": 25,
          "category": "أمثال نجدية",
          "emojis": "🍵🏠",
          "proverb": "القهوة على القهوة سيرة وسلوة",
          "meaning": "الجلسات والمجالس تُحل المشكلات وتُريح النفوس — الحوار الهادئ يُريح القلب",
          "hint": "مجلس القهوة في نجد له مكانة",
          "points": 200
        },
        {
          "id": 26,
          "category": "أمثال نجدية",
          "emojis": "🔥🪵",
          "proverb": "النار ما تحرق نفسها",
          "meaning": "الظالم لا يظلم نفسه — الإنسان بطبعه يميل لحماية مصالحه",
          "hint": "النار وطبيعتها",
          "points": 600
        },
        {
          "id": 27,
          "category": "أمثال نجدية",
          "emojis": "🤝🧑‍🤝‍🧑",
          "proverb": "يد الله مع الجماعة",
          "meaning": "القوة والبركة في الاجتماع والوحدة — التفرق يُضعف والاجتماع يُقوّي",
          "hint": "حديث نبوي أصبح مثلاً شعبياً",
          "points": 200
        },
        {
          "id": 28,
          "category": "أمثال نجدية",
          "emojis": "🗡️🐑",
          "proverb": "اذبح بسكين وتعشى بسكين",
          "meaning": "استخدم نفس الأداة لقضاء حاجتين — الشاطر يُكمل شغله بأقل الأدوات",
          "hint": "السكين أداة متعددة الأغراض",
          "points": 600
        },
        {
          "id": 29,
          "category": "أمثال نجدية",
          "emojis": "👁️🐦",
          "proverb": "الطير يعرف طيره",
          "meaning": "كل إنسان يعرف من هو على شاكلته وطبيعته — الأشباه يجتمعون",
          "hint": "الطيور على أشكالها تقع",
          "points": 200
        },
        {
          "id": 30,
          "category": "أمثال نجدية",
          "emojis": "🌊🪨",
          "proverb": "الماء يشق الصخر بالتكرار",
          "meaning": "المثابرة والتكرار يُحققان المستحيل — الصبر والتواصل يكسران الصعاب",
          "hint": "الماء اللطيف يكسر الحجر الصلب",
          "points": 200
        },
        {
          "id": 31,
          "category": "أمثال نجدية",
          "emojis": "🌙✨",
          "proverb": "الليل يجيب همومه ويروح بهمومه",
          "meaning": "الليل وقت التفكير والهموم، والصبح يُذهب كثيراً منها — الحزن مؤقت",
          "hint": "الليل وما يحمله من أفكار",
          "points": 400
        },
        {
          "id": 32,
          "category": "أمثال نجدية",
          "emojis": "🧔🪑",
          "proverb": "أهل الديرة أدرى بديرتهم",
          "meaning": "سكان المنطقة أعلم بشؤونها وطبيعتها من الغريب — أصحاب الخبرة الميدانية أعلم",
          "hint": "الديرة = المنطقة الأصلية",
          "points": 200
        },
        {
          "id": 33,
          "category": "أمثال نجدية",
          "emojis": "🧵🪡",
          "proverb": "الخيط الطويل يتعقد",
          "meaning": "الأمر اللي يطول بدون حسم يتعقد ويصعب حلّه — أحسم أمورك مبكراً",
          "hint": "الخيط والتعقيد صورة بسيطة وبليغة",
          "points": 400
        },
        {
          "id": 34,
          "category": "أمثال نجدية",
          "emojis": "🏹🎯",
          "proverb": "اللي ما له أول ما له آخر",
          "meaning": "الشيء الذي لا أساس له لن يكون له نهاية جيدة — البدايات تحدد النهايات",
          "hint": "الأول والآخر مرتبطان",
          "points": 400
        },
        {
          "id": 35,
          "category": "أمثال نجدية",
          "emojis": "🌴🐐",
          "proverb": "كل ممشى يعلّم",
          "meaning": "كل تجربة — حتى لو بسيطة — تُعلمك شيئاً جديداً — التجربة مدرسة",
          "hint": "المشي في الطريق تجربة",
          "points": 200
        },
        {
          "id": 36,
          "category": "أمثال نجدية",
          "emojis": "💨🔥",
          "proverb": "ما يطفي نار الشوق إلا الوصال",
          "meaning": "الشوق والاشتياق لا يُطفأ إلا باللقاء والقُرب — يُضرب في الغياب الطويل",
          "hint": "النار والشوق صورة واحدة",
          "points": 400
        },
        {
          "id": 37,
          "category": "أمثال نجدية",
          "emojis": "🌵🌧️🌱",
          "proverb": "بعد الجدب خصوبة",
          "meaning": "بعد الضيق والشدة يأتي الفرج والخير — الصبر على القحط يُكافأ بالمطر",
          "hint": "الجدب عكس الخصوبة في بيئة نجد",
          "points": 200
        },
        {
          "id": 38,
          "category": "أمثال نجدية",
          "emojis": "🧑‍🦳💬",
          "proverb": "كلام الكبير ولو كان في العارض يدفع",
          "meaning": "رأي الكبير في السن والتجربة له وزن وثقل حتى لو بدا بسيطاً — الخبرة لا تُقدَّر",
          "hint": "في نجد يقال ولو بالشيء البسيط فكلام الكبير له ثقل",
          "points": 600
        },
        {
          "id": 39,
          "category": "أمثال نجدية",
          "emojis": "🫀🏠",
          "proverb": "البيت اللي ما فيه رجال خراب",
          "meaning": "القيادة والحضور الفاعل في الأسرة ضرورة — غياب الرأس يُفقد الاستقرار",
          "hint": "البيت يحتاج عمود يسنده",
          "points": 400
        },
        {
          "id": 40,
          "category": "أمثال نجدية",
          "emojis": "🌡️🧊",
          "proverb": "كل برد وله نهاية",
          "meaning": "كل أزمة ومشكلة مهما طالت لها نهاية — الكرب يُقابله فرج",
          "hint": "البرد القارس في نجد له موسم",
          "points": 200
        },
        {
          "id": 41,
          "category": "أمثال نجدية",
          "emojis": "🚶👣",
          "proverb": "العجلة من الشيطان والتأني من الرحمن",
          "meaning": "التسرع يجلب الأخطاء والروية تجلب الصواب — تمهّل قبل كل قرار",
          "hint": "مثل ديني ترسّخ في نجد",
          "points": 200
        },
        {
          "id": 42,
          "category": "أمثال نجدية",
          "emojis": "🐪🧭",
          "proverb": "الجمل يشم مبارك صاحبه",
          "meaning": "الشخص الحكيم يعرف أين يقيم ويستقر — الحاسة السادسة في اختيار المكان",
          "hint": "الجمل لا ينيخ إلا في مكان فيه خير",
          "points": 600
        },
        {
          "id": 43,
          "category": "أمثال نجدية",
          "emojis": "🥣🫳",
          "proverb": "اللي ينعم بالدلة يصبّها على إيده",
          "meaning": "المتعجل أو المبالغ في استعجاله يُلحق الأذى بنفسه — التسرع يُحرق صاحبه",
          "hint": "الدلة الساخنة وخطر السرعة",
          "points": 400
        },
        {
          "id": 44,
          "category": "أمثال نجدية",
          "emojis": "🌵🌴💪",
          "proverb": "اللي يحضن الشوك ما يشكو",
          "meaning": "من يختار طريقاً صعباً أو يصاحب أهل الشر لا يحق له الشكوى من الأذى",
          "hint": "الشوك وخطر الاحتضان",
          "points": 400
        },
        {
          "id": 45,
          "category": "أمثال نجدية",
          "emojis": "🧑‍🌾🌱",
          "proverb": "مهنة الزراعة ما تخزّي",
          "meaning": "العمل الشريف مهما كان بسيطاً يحفظ الكرامة — الشغل بكل أنواعه شرف",
          "hint": "الزراعة رزق حلال في أرض نجد",
          "points": 400
        },
        {
          "id": 46,
          "category": "أمثال نجدية",
          "emojis": "🦅👁️",
          "proverb": "عين الصقر ما تنام",
          "meaning": "الحذق والانتباه الدائم سمة الناجح — اليقظة الدائمة ضرورة لمن يريد النجاح",
          "hint": "الصقر طير الصيد والتركيز",
          "points": 400
        },
        {
          "id": 47,
          "category": "أمثال نجدية",
          "emojis": "💦🫙",
          "proverb": "الماء ما يعلو على فمّه",
          "meaning": "كل شيء له حدوده الطبيعية — لا تتوقع من أحد أكثر مما يملك أو يقدر",
          "hint": "الماء يصل لمستواه الطبيعي دائماً",
          "points": 600
        },
        {
          "id": 48,
          "category": "أمثال نجدية",
          "emojis": "🔑🚪",
          "proverb": "باب النجار مخلّع",
          "meaning": "صاحب الحرفة قد يهمل شؤونه الخاصة وهو يخدم الناس — الإسكافي حافي",
          "hint": "النجار يُصلح بيوت الناس ويترك بيته",
          "points": 200
        },
        {
          "id": 49,
          "category": "أمثال نجدية",
          "emojis": "🌑🌕",
          "proverb": "ما كل ليلة تجيب قمرها",
          "meaning": "لا تتوقع الخير والفرح في كل وقت — الأيام دول وفيها الطيب والكدر",
          "hint": "القمر لا يكتمل كل ليلة",
          "points": 400
        },
        {
          "id": 50,
          "category": "أمثال نجدية",
          "emojis": "🫗🧊",
          "proverb": "اللي طفح كيله ما ينداس",
          "meaning": "من بلغ حدّه من الصبر والتحمل لا يمكن إجباره على المزيد — الكيل يطفح والإنسان يتضجر",
          "hint": "الكيل أداة قياس نجدية قديمة",
          "points": 600
        }
      ]
    },
    "huroof_waloof": {
      "name": "حروف وألوف",
      "description": "خمّن الإجابة التي تبدأ بالحرف المحدد من بين الأسئلة المتنوعة",
      "totalQuestions": 174,
      "letters": {
        "أ": [
          {
            "id": 1,
            "letter": "أ",
            "category": "جغرافيا",
            "question": "وش اسم أكبر قارة في العالم؟",
            "answer": "آسيا",
            "hint": "تضم الصين والهند واليابان",
            "points": 200
          },
          {
            "id": 2,
            "letter": "أ",
            "category": "إسلاميات",
            "question": "وش اسم أول نبي خلقه الله وأبو البشر؟",
            "answer": "آدم عليه السلام",
            "hint": "سُكن الجنة مع زوجته حواء",
            "points": 200
          },
          {
            "id": 3,
            "letter": "أ",
            "category": "ثقافة عامة",
            "question": "وش اسم شركة النفط السعودية الأكبر في العالم؟",
            "answer": "أرامكو السعودية",
            "hint": "اختصار لـ Arabian American Oil Company",
            "points": 200
          },
          {
            "id": 4,
            "letter": "أ",
            "category": "جغرافيا",
            "question": "وش اسم المدينة السعودية الجنوبية المشهورة بجبالها وضبابها وبرودتها؟",
            "answer": "أبها",
            "hint": "عاصمة منطقة عسير",
            "points": 200
          },
          {
            "id": 5,
            "letter": "أ",
            "category": "تقنية",
            "question": "وش اسم تطبيق الصور والريلز الشهير من شركة ميتا؟",
            "answer": "إنستقرام",
            "hint": "شعاره كاميرا ملونة",
            "points": 200
          },
          {
            "id": 6,
            "letter": "أ",
            "category": "سيارات",
            "question": "وش اسم شركة السيارات الألمانية الفاخرة التي شعارها أربع حلقات؟",
            "answer": "أودي",
            "hint": "تصنع A4 وQ7 وR8",
            "points": 400
          },
          {
            "id": 7,
            "letter": "أ",
            "category": "تقنية",
            "question": "وش اسم أشهر هاتف ذكي تصنعه شركة Apple؟",
            "answer": "آيفون",
            "hint": "iOS هو نظام تشغيله",
            "points": 200
          },
          {
            "id": 8,
            "letter": "أ",
            "category": "إسلاميات",
            "question": "وش اسم النبي الذي ابتُلي بالمرض الشديد وصبر فكافأه الله بالشفاء؟",
            "answer": "أيوب عليه السلام",
            "hint": "مثل يُضرب في الصبر",
            "points": 200
          },
          {
            "id": 9,
            "letter": "أ",
            "category": "شعارات ومطاعم",
            "question": "وش اسم موقع التسوق الإلكتروني الأشهر في العالم واللي كان اسمه سوق.كوم في السعودية؟",
            "answer": "أمازون",
            "hint": "شعاره سهم من A إلى Z",
            "points": 200
          },
          {
            "id": 10,
            "letter": "أ",
            "category": "ثقافة عامة",
            "question": "وش اسم أضخم منظمة دولية في العالم مقرّها نيويورك؟",
            "answer": "الأمم المتحدة",
            "hint": "تأسّست بعد الحرب العالمية الثانية",
            "points": 400
          }
        ],
        "ب": [
          {
            "id": 11,
            "letter": "ب",
            "category": "جغرافيا",
            "question": "وش اسم عاصمة العراق؟",
            "answer": "بغداد",
            "hint": "على ضفاف نهر دجلة",
            "points": 200
          },
          {
            "id": 12,
            "letter": "ب",
            "category": "شعارات ومطاعم",
            "question": "وش اسم سلسلة مطاعم البيتزا الأمريكية التي شعارها سقف أحمر؟",
            "answer": "بيتزا هت",
            "hint": "من أوائل سلاسل البيتزا العالمية",
            "points": 200
          },
          {
            "id": 13,
            "letter": "ب",
            "category": "تقنية",
            "question": "وش اسم لغة البرمجة المناسبة للمبتدئين والمشهورة في الذكاء الاصطناعي؟",
            "answer": "بايثون",
            "hint": "سُميت على اسم أفعى ضخمة",
            "points": 400
          },
          {
            "id": 14,
            "letter": "ب",
            "category": "إسلاميات",
            "question": "وش اسم المدينة المقدسة التي تضم المسجد الأقصى في فلسطين؟",
            "answer": "بيت المقدس",
            "hint": "القدس الشريف",
            "points": 200
          },
          {
            "id": 15,
            "letter": "ب",
            "category": "تاريخ",
            "question": "وش اسم البئر الذي اكتُشف فيه أول نفط تجاري في المملكة بالمنطقة الشرقية؟",
            "answer": "بئر الدمام رقم 7",
            "hint": "في الثلاثينيات الميلادية",
            "points": 600
          },
          {
            "id": 16,
            "letter": "ب",
            "category": "لهجات سعودية",
            "question": "وش اسم الدرّاجة الهوائية في اللهجة السعودية؟",
            "answer": "بسكليت",
            "hint": "من الكلمة الإنجليزية bicycle",
            "points": 200
          },
          {
            "id": 17,
            "letter": "ب",
            "category": "ثقافة عامة",
            "question": "وش اسم الشاعر النجدي السعودي الأمير الذي تُوفي مؤخراً وعُرف بقصائد الحنين؟",
            "answer": "بدر بن عبدالمحسن",
            "hint": "أمير وشاعر وفنان",
            "points": 400
          },
          {
            "id": 18,
            "letter": "ب",
            "category": "الدوري السعودي",
            "question": "وش اسم أشهر نادي رياضي سعودي مقرّه جدة بألوان أصفر وأسود؟",
            "answer": "بطل الاتحاد",
            "hint": "نادي جدة المشهور بألوانه المميزة",
            "points": 200
          }
        ],
        "ت": [
          {
            "id": 19,
            "letter": "ت",
            "category": "تقنية",
            "question": "وش اسم التطبيق الصيني للفيديوهات القصيرة الأشهر عند الشباب؟",
            "answer": "تيك توك",
            "hint": "ByteDance هي الشركة المطوّرة",
            "points": 200
          },
          {
            "id": 20,
            "letter": "ت",
            "category": "سيارات",
            "question": "وش اسم الشركة اليابانية التي تصنع سيارة كورولا وكامري ولاند كروزر؟",
            "answer": "تويوتا",
            "hint": "من أكثر شركات السيارات مبيعاً في العالم",
            "points": 200
          },
          {
            "id": 21,
            "letter": "ت",
            "category": "تقنية",
            "question": "وش اسم التطبيق الحكومي السعودي الذي استُخدم في تصاريح التنقل خلال كورونا؟",
            "answer": "توكّلنا",
            "hint": "يعرض التطعيم والتصاريح",
            "points": 200
          },
          {
            "id": 22,
            "letter": "ت",
            "category": "إسلاميات",
            "question": "وش اسم التمر الشهير من المدينة المنورة الذي قال عنه النبي ﷺ إنه شفاء؟",
            "answer": "تمر العجوة",
            "hint": "أسود اللون من أجود أنواع التمور",
            "points": 200
          },
          {
            "id": 23,
            "letter": "ت",
            "category": "تاريخ",
            "question": "وش اسم أول محطة تلفزيونية رسمية في المملكة العربية السعودية؟",
            "answer": "تلفزيون السعودية",
            "hint": "بدأ بثّه عام 1965م",
            "points": 400
          },
          {
            "id": 24,
            "letter": "ت",
            "category": "جغرافيا",
            "question": "وش اسم المدينة السعودية الشمالية التي تشتهر بآثار الحجر وعالم الأنباط؟",
            "answer": "تبوك",
            "hint": "شمال غرب المملكة على الخليج العقبي",
            "points": 400
          },
          {
            "id": 25,
            "letter": "ت",
            "category": "ثقافة عامة",
            "question": "وش اسم مبنى هندي مشهور بنّاه الإمبراطور شاه جهان لزوجته؟",
            "answer": "تاج محل",
            "hint": "يقع في آغرا بالهند",
            "points": 200
          }
        ],
        "ث": [
          {
            "id": 26,
            "letter": "ث",
            "category": "لهجات سعودية",
            "question": "وش اسم اللبس الرجالي التقليدي السعودي الطويل الأبيض؟",
            "answer": "ثوب",
            "hint": "يُلبس مع الغترة أو الشماغ",
            "points": 200
          },
          {
            "id": 27,
            "letter": "ث",
            "category": "ثقافة عامة",
            "question": "وش اسم الحيوان الذكي الماكر الذي يُضرب به المثل في الدهاء؟",
            "answer": "ثعلب",
            "hint": "يظهر في الحكايات الشعبية كثيراً",
            "points": 200
          },
          {
            "id": 28,
            "letter": "ث",
            "category": "ثقافة عامة",
            "question": "وش اسم الجهاز المنزلي الأساسي لحفظ الطعام بارداً؟",
            "answer": "ثلاجة",
            "hint": "اخترعه الإنسان في القرن العشرين",
            "points": 200
          },
          {
            "id": 29,
            "letter": "ث",
            "category": "إسلاميات",
            "question": "وش اسم مجموعة النجوم الشهيرة في السماء العربية والمشار إليها في القرآن؟",
            "answer": "الثريا",
            "hint": "تتكون من نجوم متقاربة في كوكبة الثور",
            "points": 400
          }
        ],
        "ج": [
          {
            "id": 30,
            "letter": "ج",
            "category": "جغرافيا",
            "question": "وش اسم المدينة السعودية التي تُلقب بعروس البحر الأحمر؟",
            "answer": "جدة",
            "hint": "ثاني أكبر مدينة في المملكة",
            "points": 200
          },
          {
            "id": 31,
            "letter": "ج",
            "category": "تقنية",
            "question": "وش اسم أشهر محرك بحث في العالم؟",
            "answer": "جوجل",
            "hint": "Google بالعربية",
            "points": 200
          },
          {
            "id": 32,
            "letter": "ج",
            "category": "جغرافيا",
            "question": "وش اسم أكبر جزيرة في العالم؟",
            "answer": "جرينلاند",
            "hint": "تتبع المملكة الدنماركية",
            "points": 400
          },
          {
            "id": 33,
            "letter": "ج",
            "category": "جغرافيا",
            "question": "وش اسم أعلى جبل في المملكة العربية السعودية الواقع في منطقة عسير؟",
            "answer": "جبل السودة",
            "hint": "يبلغ ارتفاعه 3015 متر",
            "points": 400
          },
          {
            "id": 34,
            "letter": "ج",
            "category": "سيارات",
            "question": "وش اسم شركة السيارات الأمريكية المشهورة بالطرق الوعرة وشعارها سبع فتحات؟",
            "answer": "جيب",
            "hint": "Jeep بالعربية",
            "points": 200
          },
          {
            "id": 35,
            "letter": "ج",
            "category": "تاريخ",
            "question": "وش اسم مهرجان التراث والثقافة السعودي السنوي الذي يُقام شمال الرياض؟",
            "answer": "الجنادرية",
            "hint": "يعكس تراث مناطق المملكة كلها",
            "points": 200
          },
          {
            "id": 36,
            "letter": "ج",
            "category": "ثقافة عامة",
            "question": "وش اسم المنظمة الدولية للصحة الأم التابعة للأمم المتحدة؟",
            "answer": "جهاز الصحة العالمي WHO",
            "hint": "اختصارها WHO وتتخذ من جنيف مقراً",
            "points": 400
          },
          {
            "id": 37,
            "letter": "ج",
            "category": "إسلاميات",
            "question": "وش اسم ملَك الوحي الذي نزل على الأنبياء والمذكور في القرآن؟",
            "answer": "جبريل عليه السلام",
            "hint": "أمين الوحي",
            "points": 200
          }
        ],
        "ح": [
          {
            "id": 38,
            "letter": "ح",
            "category": "إسلاميات",
            "question": "وش اسم الركن الخامس والأخير من أركان الإسلام؟",
            "answer": "حج بيت الله الحرام",
            "hint": "فريضة على كل مسلم مستطيع",
            "points": 200
          },
          {
            "id": 39,
            "letter": "ح",
            "category": "جغرافيا",
            "question": "وش اسم المدينة السعودية الشمالية المشهورة بقلعتها التاريخية وطبيعتها؟",
            "answer": "حائل",
            "hint": "عاصمة المنطقة التي تحمل اسمها",
            "points": 200
          },
          {
            "id": 40,
            "letter": "ح",
            "category": "ثقافة عامة",
            "question": "وش اسم الأكلة السعودية التقليدية من القمح المهروس مع اللحم التي تُقدم في المناسبات؟",
            "answer": "حريسة",
            "hint": "طبق شعبي يُعمل على نار هادئة",
            "points": 200
          },
          {
            "id": 41,
            "letter": "ح",
            "category": "ثقافة عامة",
            "question": "وش اسم أكبر حيوان بحري في العالم؟",
            "answer": "حوت أزرق",
            "hint": "يمكن أن يصل طوله لـ 30 متراً",
            "points": 200
          },
          {
            "id": 42,
            "letter": "ح",
            "category": "إسلاميات",
            "question": "وش اسم الحبّة السوداء التي قال عنها النبي ﷺ إنها شفاء من كل داء إلا الموت؟",
            "answer": "حبة البركة",
            "hint": "تُعرف أيضاً بالحبة السوداء",
            "points": 200
          },
          {
            "id": 43,
            "letter": "ح",
            "category": "الدوري السعودي",
            "question": "وش اسم البطولة العالمية التي احتضنتها قطر لأول مرة في العالم العربي؟",
            "answer": "حفلة كأس العالم 2022",
            "hint": "في الدوحة عام 2022",
            "points": 200
          },
          {
            "id": 44,
            "letter": "ح",
            "category": "تاريخ",
            "question": "وش اسم معركة خاضها النبي ﷺ في السنة الثالثة الهجرية بجبل قريب من المدينة؟",
            "answer": "حرب أحد - غزوة أحد",
            "hint": "جبله موجود في المدينة المنورة",
            "points": 400
          }
        ],
        "خ": [
          {
            "id": 45,
            "letter": "خ",
            "category": "جغرافيا",
            "question": "وش اسم الذراع المائي الذي يفصل المنطقة الشرقية السعودية عن إيران؟",
            "answer": "الخليج العربي",
            "hint": "يُسمى أيضاً الخليج الفارسي",
            "points": 200
          },
          {
            "id": 46,
            "letter": "خ",
            "category": "ثقافة عامة",
            "question": "وش اسم شركة الطيران الوطنية السعودية؟",
            "answer": "الخطوط السعودية",
            "hint": "شعارها نخلة وسيوف",
            "points": 200
          },
          {
            "id": 47,
            "letter": "خ",
            "category": "ثقافة عامة",
            "question": "وش اسم مرحلة نضج التمر حين يكون أخضر اللون؟",
            "answer": "خلال",
            "hint": "قبل أن يصبح رطباً ثم تمراً",
            "points": 400
          },
          {
            "id": 48,
            "letter": "خ",
            "category": "جغرافيا",
            "question": "وش اسم المدينة الساحلية في المنطقة الشرقية المجاورة للدمام؟",
            "answer": "الخبر",
            "hint": "تربطها بالبحرين جسر الملك فهد",
            "points": 200
          },
          {
            "id": 49,
            "letter": "خ",
            "category": "إسلاميات",
            "question": "وش اسم الخليفة الراشدي الثالث الذي جمع القرآن الكريم؟",
            "answer": "خليفة عثمان بن عفان",
            "hint": "ذو النورين",
            "points": 400
          }
        ],
        "د": [
          {
            "id": 50,
            "letter": "د",
            "category": "جغرافيا",
            "question": "وش اسم أشهر مدينة في الإمارات المشهورة بناطحات السحاب؟",
            "answer": "دبي",
            "hint": "فيها برج خليفة أطول برج في العالم",
            "points": 200
          },
          {
            "id": 51,
            "letter": "د",
            "category": "ثقافة عامة",
            "question": "وش اسم إناء القهوة العربية النحاسي الشهير؟",
            "answer": "دلة القهوة",
            "hint": "رمز الكرم في البيت النجدي",
            "points": 200
          },
          {
            "id": 52,
            "letter": "د",
            "category": "الدوري السعودي",
            "question": "وش اسم الدوري السعودي للمحترفين بعد تغيير الراعي لشركة عقارية؟",
            "answer": "دوري روشن للمحترفين",
            "hint": "روشن شركة تطوير عقاري",
            "points": 400
          },
          {
            "id": 53,
            "letter": "د",
            "category": "ثقافة عامة",
            "question": "وش اسم الحيوان الثديي المائي المعروف بذكائه وصوته المميز؟",
            "answer": "دلفين",
            "hint": "يعيش في المحيطات ويتفاعل مع البشر",
            "points": 200
          },
          {
            "id": 54,
            "letter": "د",
            "category": "تاريخ",
            "question": "وش اسم المدينة التاريخية التي كانت عاصمة الدولة السعودية الأولى؟",
            "answer": "الدرعية",
            "hint": "موجودة في شمال الرياض وأصبحت منطقة تراثية",
            "points": 400
          }
        ],
        "ر": [
          {
            "id": 55,
            "letter": "ر",
            "category": "جغرافيا",
            "question": "وش اسم عاصمة المملكة العربية السعودية؟",
            "answer": "الرياض",
            "hint": "أكبر مدينة في المملكة",
            "points": 200
          },
          {
            "id": 56,
            "letter": "ر",
            "category": "الدوري السعودي",
            "question": "وش اسم النجم البرتغالي الذي انضم لنادي النصر السعودي عام 2023؟",
            "answer": "رونالدو",
            "hint": "CR7 أحد أعظم لاعبي كرة القدم",
            "points": 200
          },
          {
            "id": 57,
            "letter": "ر",
            "category": "إسلاميات",
            "question": "وش اسم شهر الصيام والقرآن في التقويم الهجري؟",
            "answer": "رمضان",
            "hint": "التاسع من أشهر السنة الهجرية",
            "points": 200
          },
          {
            "id": 58,
            "letter": "ر",
            "category": "تقنية",
            "question": "وش اسم الآلة الذكية التي تُصمَّم لتحاكي حركات وقدرات الإنسان؟",
            "answer": "روبوت",
            "hint": "تستخدمه المصانع والمستشفيات",
            "points": 200
          },
          {
            "id": 59,
            "letter": "ر",
            "category": "الدوري السعودي",
            "question": "وش اسم الراعي الرسمي للدوري السعودي للمحترفين حالياً؟",
            "answer": "روشن",
            "hint": "شركة تطوير عقاري سعودية",
            "points": 400
          },
          {
            "id": 60,
            "letter": "ر",
            "category": "إسلاميات",
            "question": "وش اسم الصوفية الإسلامية المشهورة من البصرة المعروفة بالحب الإلهي؟",
            "answer": "رابعة العدوية",
            "hint": "من أشهر العابدات في التاريخ الإسلامي",
            "points": 600
          }
        ],
        "ز": [
          {
            "id": 61,
            "letter": "ز",
            "category": "محافظة الزلفي",
            "question": "وش اسم محافظة سعودية في منطقة الرياض تشتهر بتمورها ونخيلها؟",
            "answer": "الزلفي",
            "hint": "تقع شمال غرب الرياض بنحو 270 كم",
            "points": 200
          },
          {
            "id": 62,
            "letter": "ز",
            "category": "إسلاميات",
            "question": "وش اسم البئر المقدسة في المسجد الحرام بمكة المكرمة؟",
            "answer": "زمزم",
            "hint": "نبعت لهاجر وإسماعيل",
            "points": 200
          },
          {
            "id": 63,
            "letter": "ز",
            "category": "ثقافة عامة",
            "question": "وش اسم التوابل الصفراء الغالية التي تُستخدم في القهوة العربية وتُعطيها لوناً ذهبياً؟",
            "answer": "زعفران",
            "hint": "أغلى بهارات العالم",
            "points": 200
          },
          {
            "id": 64,
            "letter": "ز",
            "category": "ثقافة عامة",
            "question": "وش اسم الحبوب الدهنية الخضراء التي تُستخدم في الطعام وعصره زيت مشهور؟",
            "answer": "زيتون",
            "hint": "شجرته مباركة ذُكرت في القرآن",
            "points": 200
          },
          {
            "id": 65,
            "letter": "ز",
            "category": "جغرافيا",
            "question": "وش اسم الظاهرة الطبيعية التي تتحرك فيها طبقات الأرض وتُسبب الدمار؟",
            "answer": "زلزال",
            "hint": "تقيسه أجهزة السيزموغراف",
            "points": 200
          }
        ],
        "س": [
          {
            "id": 66,
            "letter": "س",
            "category": "مسلسلات سعودية",
            "question": "وش اسم البرنامج الكوميدي السعودي الذي يُقدمه ناصر القصبي ويعكس الحياة اليومية؟",
            "answer": "سيلفي",
            "hint": "يعرض على قناة MBC",
            "points": 200
          },
          {
            "id": 67,
            "letter": "س",
            "category": "تقنية",
            "question": "وش اسم الشركة الكورية المنافسة لآبل صاحبة هاتف Galaxy؟",
            "answer": "سامسونج",
            "hint": "شركة إلكترونيات كورية ضخمة",
            "points": 200
          },
          {
            "id": 68,
            "letter": "س",
            "category": "جغرافيا",
            "question": "وش اسم دولة جنوب آسيا المشهورة بمطارها الأكثر كفاءة واقتصادها المتقدم؟",
            "answer": "سنغافورة",
            "hint": "دولة مدينة على جزيرة صغيرة",
            "points": 400
          },
          {
            "id": 69,
            "letter": "س",
            "category": "إسلاميات",
            "question": "وش اسم النبي الذي وُهب ملكاً عظيماً وكان يُكلّم الطير والحيوان؟",
            "answer": "سليمان عليه السلام",
            "hint": "ابن داود عليه السلام",
            "points": 200
          },
          {
            "id": 70,
            "letter": "س",
            "category": "تقنية",
            "question": "وش اسم التطبيق الذي تختفي فيه الرسائل والصور بعد وقت محدد؟",
            "answer": "سناب تشات",
            "hint": "شعاره روح صفراء",
            "points": 200
          },
          {
            "id": 71,
            "letter": "س",
            "category": "جغرافيا",
            "question": "وش اسم السلطنة العربية الجارة للمملكة من الجنوب الشرقي؟",
            "answer": "سلطنة عُمان",
            "hint": "عاصمتها مسقط",
            "points": 200
          },
          {
            "id": 72,
            "letter": "س",
            "category": "سيارات",
            "question": "وش اسم الشركة اليابانية صاحبة سيارة أوتباك وفورستر بنظام الدفع الرباعي؟",
            "answer": "سوبارو",
            "hint": "شعارها مجموعة نجوم الثريا",
            "points": 400
          }
        ],
        "ش": [
          {
            "id": 73,
            "letter": "ش",
            "category": "لهجات سعودية",
            "question": "وش اسم الغطاء الرأسي السعودي المربعات الحمراء والبيضاء؟",
            "answer": "شماغ",
            "hint": "يُلبس مع العقال",
            "points": 200
          },
          {
            "id": 74,
            "letter": "ش",
            "category": "تقنية",
            "question": "وش اسم منصة البث التلفزيوني الرقمي العربية من مجموعة MBC؟",
            "answer": "شاهد",
            "hint": "تنافس نتفليكس في المحتوى العربي",
            "points": 200
          },
          {
            "id": 75,
            "letter": "ش",
            "category": "ثقافة عامة",
            "question": "وش اسم شاعر مصري يُلقب بـ'أمير الشعراء' في العصر الحديث؟",
            "answer": "شوقي - أحمد شوقي",
            "hint": "صاحب 'الشعراء ورثة الأنبياء'",
            "points": 400
          },
          {
            "id": 76,
            "letter": "ش",
            "category": "إسلاميات",
            "question": "وش اسم الشهر الهجري الثامن الذي يسبق رمضان مباشرةً؟",
            "answer": "شعبان",
            "hint": "يصومه النبي ﷺ كثيراً",
            "points": 400
          },
          {
            "id": 77,
            "letter": "ش",
            "category": "شعارات ومطاعم",
            "question": "وش اسم أكلة الشوارع الشعبية المشهورة في السعودية من اللحم المشوي مع الخبز؟",
            "answer": "شاورما",
            "hint": "من أكثر الأكلات الشعبية انتشاراً",
            "points": 200
          },
          {
            "id": 78,
            "letter": "ش",
            "category": "مسلسلات سعودية",
            "question": "وش اسم قناة التلفزيون السعودية الترفيهية الأشهر للشباب؟",
            "answer": "شاشة MBC",
            "hint": "بثّها من دبي والرياض",
            "points": 200
          },
          {
            "id": 79,
            "letter": "ش",
            "category": "محافظة الزلفي",
            "question": "وش اسم ظاهرة الطبيعة التي تجعل الزلفي أرضاً خضراء في موسم الأمطار؟",
            "answer": "شبكة أودية الزلفي",
            "hint": "الأمطار تملأ الأودية وتحيي الأرض",
            "points": 600
          }
        ],
        "ص": [
          {
            "id": 80,
            "letter": "ص",
            "category": "ثقافة عامة",
            "question": "وش اسم الطائر النبيل الذي يُستخدم في الصيد التقليدي لدى الأمراء والعرب؟",
            "answer": "صقر",
            "hint": "رياضة الصقارة تراث إنساني",
            "points": 200
          },
          {
            "id": 81,
            "letter": "ص",
            "category": "إسلاميات",
            "question": "وش اسم النبي الذي أُرسل لقوم ثمود أصحاب الناقة؟",
            "answer": "صالح عليه السلام",
            "hint": "قومه كانوا ينحتون البيوت في الجبال",
            "points": 200
          },
          {
            "id": 82,
            "letter": "ص",
            "category": "ثقافة عامة",
            "question": "وش اسم فصل السنة الذي تشتد فيه الحرارة ويطول فيه النهار؟",
            "answer": "صيف",
            "hint": "يسبقه الربيع ويليه الخريف",
            "points": 200
          },
          {
            "id": 83,
            "letter": "ص",
            "category": "لهجات سعودية",
            "question": "وش معنى كلمة 'صج' في اللهجة النجدية السعودية؟",
            "answer": "صحيح وحقيقي",
            "hint": "تقولها للتأكيد على الكلام",
            "points": 200
          },
          {
            "id": 84,
            "letter": "ص",
            "category": "شعارات ومطاعم",
            "question": "وش اسم السلسلة السعودية الأشهر للأكلات الشعبية من دجاج وروبيان؟",
            "answer": "صفقة البيك",
            "hint": "سلسلة سعودية أصيلة وأسعارها شعبية",
            "points": 200
          }
        ],
        "ط": [
          {
            "id": 85,
            "letter": "ط",
            "category": "جغرافيا",
            "question": "وش اسم عاصمة ليبيا؟",
            "answer": "طرابلس",
            "hint": "مدينة ساحلية على البحر الأبيض المتوسط",
            "points": 400
          },
          {
            "id": 86,
            "letter": "ط",
            "category": "مسلسلات سعودية",
            "question": "وش اسم المسلسل السعودي الكوميدي الشهير الذي استمر 20 سنة بين ناصر القصبي وعبدالله السدحان؟",
            "answer": "طاش ما طاش",
            "hint": "عُرض في رمضان على MBC",
            "points": 200
          },
          {
            "id": 87,
            "letter": "ط",
            "category": "شعارات ومطاعم",
            "question": "وش اسم صلصة السمسم الشهيرة التي تُستخدم في الحمص والسلطات؟",
            "answer": "طحينة",
            "hint": "تُصنع من حبوب السمسم المطحونة",
            "points": 200
          },
          {
            "id": 88,
            "letter": "ط",
            "category": "تقنية",
            "question": "وش اسم الطائرات التي تُحرَّك عن بُعد بدون طيار؟",
            "answer": "طائرات مسيّرة",
            "hint": "Drones بالإنجليزية",
            "points": 200
          },
          {
            "id": 89,
            "letter": "ط",
            "category": "إسلاميات",
            "question": "وش اسم الطريق الذي سلكه النبي ﷺ في هجرته من مكة للمدينة المنورة؟",
            "answer": "طريق الهجرة النبوية",
            "hint": "مسار تاريخي يُسلك اليوم سياحياً",
            "points": 400
          }
        ],
        "ع": [
          {
            "id": 90,
            "letter": "ع",
            "category": "جغرافيا",
            "question": "وش اسم عاصمة الأردن؟",
            "answer": "عمّان",
            "hint": "مبنية على سبعة تلال",
            "points": 200
          },
          {
            "id": 91,
            "letter": "ع",
            "category": "إسلاميات",
            "question": "وش اسم الجبل الذي يقف عنده الحجاج في أعظم يوم من أيام الحج؟",
            "answer": "عرفة",
            "hint": "الحج عرفة كما في الحديث النبوي",
            "points": 200
          },
          {
            "id": 92,
            "letter": "ع",
            "category": "تاريخ",
            "question": "وش اسم الخليفة الراشدي الثاني المشهور بعدله وقوله 'لو عثرت بغلة في العراق'؟",
            "answer": "عمر بن الخطاب رضي الله عنه",
            "hint": "الفاروق",
            "points": 200
          },
          {
            "id": 93,
            "letter": "ع",
            "category": "ثقافة عامة",
            "question": "وش اسم الرقصة الشعبية السعودية التي تُؤدى بالسيوف في المناسبات والأعياد؟",
            "answer": "عرضة سعودية",
            "hint": "أُدرجت في قائمة التراث الإنساني لليونسكو",
            "points": 200
          },
          {
            "id": 94,
            "letter": "ع",
            "category": "تاريخ",
            "question": "وش اسم العصر الذهبي للحضارة الإسلامية في العلوم والفلسفة والأدب؟",
            "answer": "العصر العباسي",
            "hint": "امتد من القرن الثامن للثالث عشر الميلادي",
            "points": 400
          },
          {
            "id": 95,
            "letter": "ع",
            "category": "ثقافة عامة",
            "question": "وش اسم العلم الذي يدرس النجوم والكواكب والكون؟",
            "answer": "علم الفلك",
            "hint": "عالمه يُسمى فلكياً",
            "points": 200
          },
          {
            "id": 96,
            "letter": "ع",
            "category": "الدوري السعودي",
            "question": "وش اسم اللاعب السعودي الأسطورة الذي سجّل أكبر عدد من الأهداف الدولية للمنتخب؟",
            "answer": "عادل خميس",
            "hint": "أسطورة الهلال والمنتخب السعودي من الثمانينيات",
            "points": 400
          },
          {
            "id": 97,
            "letter": "ع",
            "category": "إسلاميات",
            "question": "وش اسم علم الشريعة الذي يدرس أحكام الحلال والحرام والفتاوى؟",
            "answer": "علم الفقه الإسلامي",
            "hint": "المتخصص فيه يُسمى فقيهاً",
            "points": 400
          },
          {
            "id": 98,
            "letter": "ع",
            "category": "محافظة الزلفي",
            "question": "وش اسم الموسم الذي تُفتح فيه مزارع التمر في الزلفي للزوار؟",
            "answer": "عروض مهرجان تمور الزلفي",
            "hint": "يُقام في موسم الحصاد",
            "points": 400
          }
        ],
        "ف": [
          {
            "id": 99,
            "letter": "ف",
            "category": "جغرافيا",
            "question": "وش اسم الأرض المقدسة التي تضم المسجد الأقصى والقدس الشريف؟",
            "answer": "فلسطين",
            "hint": "قضية العرب والمسلمين الأولى",
            "points": 200
          },
          {
            "id": 100,
            "letter": "ف",
            "category": "سيارات",
            "question": "وش اسم شركة السيارات الإيطالية الشهيرة التي شعارها حصان أسود على خلفية صفراء؟",
            "answer": "فيراري",
            "hint": "مقرها في مدينة مارانيلو",
            "points": 200
          },
          {
            "id": 101,
            "letter": "ف",
            "category": "تقنية",
            "question": "وش اسم أشهر شبكة تواصل اجتماعي في العالم أسسها مارك زوكربيرغ؟",
            "answer": "فيسبوك",
            "hint": "Meta هي الشركة الأم الآن",
            "points": 200
          },
          {
            "id": 102,
            "letter": "ف",
            "category": "إسلاميات",
            "question": "وش اسم السورة الأولى في القرآن الكريم وأم الكتاب؟",
            "answer": "الفاتحة",
            "hint": "تُقرأ في كل ركعة من ركعات الصلاة",
            "points": 200
          },
          {
            "id": 103,
            "letter": "ف",
            "category": "الدوري السعودي",
            "question": "وش اسم اللاعب السعودي الذي سجّل أول أهداف المنتخب على الأرجنتين في مونديال 2022؟",
            "answer": "فراس البريكان",
            "hint": "لاعب نادي الاتحاد",
            "points": 400
          }
        ],
        "ق": [
          {
            "id": 104,
            "letter": "ق",
            "category": "جغرافيا",
            "question": "وش اسم الدولة الخليجية التي استضافت كأس العالم 2022؟",
            "answer": "قطر",
            "hint": "عاصمتها الدوحة",
            "points": 200
          },
          {
            "id": 105,
            "letter": "ق",
            "category": "إسلاميات",
            "question": "وش اسم القبيلة التي ينتمي إليها النبي محمد ﷺ؟",
            "answer": "قريش",
            "hint": "سيدة قبائل العرب",
            "points": 200
          },
          {
            "id": 106,
            "letter": "ق",
            "category": "إسلاميات",
            "question": "وش اسم البناء الإسلامي الشهير في القدس ذو القبة الذهبية فوق الصخرة؟",
            "answer": "قبة الصخرة",
            "hint": "بُني في العهد الأموي",
            "points": 400
          },
          {
            "id": 107,
            "letter": "ق",
            "category": "ثقافة عامة",
            "question": "وش اسم مشروع الترفيه السعودي الضخم الذي سيُبنى على بُعد 40 كم من الرياض؟",
            "answer": "القدية",
            "hint": "من مشاريع صندوق الاستثمارات العامة",
            "points": 400
          },
          {
            "id": 108,
            "letter": "ق",
            "category": "جغرافيا",
            "question": "وش اسم المنطقة السعودية الشهيرة بزراعة القمح والرمان شمال المملكة؟",
            "answer": "القصيم",
            "hint": "عاصمتها بريدة",
            "points": 200
          },
          {
            "id": 109,
            "letter": "ق",
            "category": "ثقافة عامة",
            "question": "وش اسم الآلة الموسيقية الوترية العربية ذات شكل المعيّن التي تُعزف بريشة؟",
            "answer": "قانون",
            "hint": "من أقدم الآلات الموسيقية العربية",
            "points": 400
          }
        ],
        "ك": [
          {
            "id": 110,
            "letter": "ك",
            "category": "ثقافة عامة",
            "question": "وش اسم الأكلة السعودية الوطنية المصنوعة من الأرز واللحم والتوابل؟",
            "answer": "كبسة",
            "hint": "أشهر أكلة سعودية",
            "points": 200
          },
          {
            "id": 111,
            "letter": "ك",
            "category": "شعارات ومطاعم",
            "question": "وش اسم السلسلة السعودية المحلية المشهورة بالبرغر والدجاج وشعارها وعل؟",
            "answer": "كودو",
            "hint": "سلسلة سعودية 100%",
            "points": 200
          },
          {
            "id": 112,
            "letter": "ك",
            "category": "سيارات",
            "question": "وش اسم شركة السيارات الكورية صاحبة موديلات K5 وK8 وسبورتاج؟",
            "answer": "كيا",
            "hint": "شركة كورية جنوبية",
            "points": 200
          },
          {
            "id": 113,
            "letter": "ك",
            "category": "الدوري السعودي",
            "question": "وش اسم اللاعب الفرنسي الفائز بالكرة الذهبية 2022 الذي انضم لنادي الهلال؟",
            "answer": "كريم بنزيمة",
            "hint": "Real Madrid كان ناديه السابق",
            "points": 200
          },
          {
            "id": 114,
            "letter": "ك",
            "category": "إسلاميات",
            "question": "وش اسم البناء المكعب الشكل في وسط المسجد الحرام الذي يطوف حوله الحجاج؟",
            "answer": "الكعبة المشرفة",
            "hint": "قبلة المسلمين في كل أنحاء العالم",
            "points": 200
          }
        ],
        "ل": [
          {
            "id": 115,
            "letter": "ل",
            "category": "جغرافيا",
            "question": "وش اسم عاصمة المملكة المتحدة (بريطانيا)؟",
            "answer": "لندن",
            "hint": "عاصمة الضباب",
            "points": 200
          },
          {
            "id": 116,
            "letter": "ل",
            "category": "تقنية",
            "question": "وش اسم نظام التشغيل المفتوح المصدر الشهير الذي يستخدمه المطورون؟",
            "answer": "لينوكس",
            "hint": "بديل مجاني لـ Windows",
            "points": 400
          },
          {
            "id": 117,
            "letter": "ل",
            "category": "إسلاميات",
            "question": "وش اسم الليلة المقدسة في رمضان التي قال عنها القرآن إنها خير من ألف شهر؟",
            "answer": "ليلة القدر",
            "hint": "تُلتمس في العشر الأواخر",
            "points": 200
          },
          {
            "id": 118,
            "letter": "ل",
            "category": "ثقافة عامة",
            "question": "وش اسم حلوى الخليج الشهيرة المقلية بالعجين الدائري مع الشيرة؟",
            "answer": "لقيمات",
            "hint": "تُقدم ساخنة مع الشيرة أو العسل",
            "points": 200
          },
          {
            "id": 119,
            "letter": "ل",
            "category": "سيارات",
            "question": "وش اسم العلامة التجارية الفاخرة الفرعية من تويوتا؟",
            "answer": "لكزس",
            "hint": "Lexus بالعربية",
            "points": 400
          }
        ],
        "م": [
          {
            "id": 120,
            "letter": "م",
            "category": "جغرافيا",
            "question": "وش اسم أكبر محيط في العالم؟",
            "answer": "المحيط الهادئ",
            "hint": "يغطي نصف سطح الكرة الأرضية تقريباً",
            "points": 400
          },
          {
            "id": 121,
            "letter": "م",
            "category": "إسلاميات",
            "question": "وش اسم ثاني أقدس مدينة في الإسلام بعد مكة المكرمة؟",
            "answer": "المدينة المنورة",
            "hint": "فيها المسجد النبوي الشريف",
            "points": 200
          },
          {
            "id": 122,
            "letter": "م",
            "category": "تاريخ",
            "question": "وش اسم مؤسس الدولة السعودية الأولى في الدرعية؟",
            "answer": "محمد بن سعود رحمه الله",
            "hint": "تحالف مع الشيخ محمد بن عبد الوهاب",
            "points": 400
          },
          {
            "id": 123,
            "letter": "م",
            "category": "الدوري السعودي",
            "question": "وش اسم اللاعب الأرجنتيني الذي يُعتبر من أعظم لاعبي تاريخ كرة القدم؟",
            "answer": "ميسي",
            "hint": "فاز بثمان كرات ذهبية",
            "points": 200
          },
          {
            "id": 124,
            "letter": "م",
            "category": "تقنية",
            "question": "وش اسم شركة التكنولوجيا الأمريكية صاحبة نظام التشغيل Windows؟",
            "answer": "مايكروسوفت",
            "hint": "أسسها بيل غيتس",
            "points": 200
          },
          {
            "id": 125,
            "letter": "م",
            "category": "ثقافة عامة",
            "question": "وش اسم المهرجان الترفيهي الضخم الذي يُقام في الرياض لأشهر متواصلة؟",
            "answer": "موسم الرياض",
            "hint": "من أضخم الفعاليات الترفيهية عربياً",
            "points": 200
          },
          {
            "id": 126,
            "letter": "م",
            "category": "شعارات ومطاعم",
            "question": "وش اسم سلسلة الوجبات السريعة الأمريكية صاحبة شعار حرف M الذهبي؟",
            "answer": "ماكدونالدز",
            "hint": "أشهر سلسلة برغر في العالم",
            "points": 200
          },
          {
            "id": 127,
            "letter": "م",
            "category": "ثقافة عامة",
            "question": "وش اسم المبنى الضخم في شنغهاي الذي يُعدّ ثاني أطول برج في العالم؟",
            "answer": "مبنى شنغهاي تاور",
            "hint": "ارتفاعه 632 متراً",
            "points": 600
          },
          {
            "id": 128,
            "letter": "م",
            "category": "محافظة الزلفي",
            "question": "وش اسم أبرز منتج زراعي تشتهر وتُعرف به محافظة الزلفي؟",
            "answer": "مزارع التمور النجدية",
            "hint": "الزلفي من أشهر مناطق إنتاج التمر",
            "points": 400
          },
          {
            "id": 129,
            "letter": "م",
            "category": "الدوري السعودي",
            "question": "وش اسم ملعب منتخب المملكة العربية السعودية الوطني في الرياض؟",
            "answer": "ملعب الملك فهد الدولي",
            "hint": "يتسع لأكثر من 68 ألف متفرج",
            "points": 200
          },
          {
            "id": 130,
            "letter": "م",
            "category": "سيارات",
            "question": "وش اسم شركة السيارات الألمانية الفاخرة التي شعارها نجمة ثلاثية الرؤوس؟",
            "answer": "مرسيدس بنز",
            "hint": "من أقدم وأشهر شركات السيارات",
            "points": 200
          }
        ],
        "ن": [
          {
            "id": 131,
            "letter": "ن",
            "category": "جغرافيا",
            "question": "وش اسم أطول نهر في العالم الذي يمر بمصر؟",
            "answer": "النيل",
            "hint": "يصب في البحر المتوسط",
            "points": 200
          },
          {
            "id": 132,
            "letter": "ن",
            "category": "الدوري السعودي",
            "question": "وش اسم اللاعب البرازيلي الشهير الذي انضم لنادي الهلال السعودي؟",
            "answer": "نيمار",
            "hint": "PSG كان ناديه السابق",
            "points": 200
          },
          {
            "id": 133,
            "letter": "ن",
            "category": "تقنية",
            "question": "وش اسم منصة البث الأمريكية الأشهر للأفلام والمسلسلات في العالم؟",
            "answer": "نتفليكس",
            "hint": "جاءت بثورة في عالم المحتوى الرقمي",
            "points": 200
          },
          {
            "id": 134,
            "letter": "ن",
            "category": "ثقافة عامة",
            "question": "وش اسم الروائي المصري الوحيد الفائز بجائزة نوبل للأدب؟",
            "answer": "نجيب محفوظ",
            "hint": "صاحب ثلاثية القاهرة",
            "points": 400
          },
          {
            "id": 135,
            "letter": "ن",
            "category": "إسلاميات",
            "question": "وش اسم النبي الذي أمره الله بصنع السفينة الكبيرة لينجو من الطوفان؟",
            "answer": "نوح عليه السلام",
            "hint": "من أولي العزم من الرسل",
            "points": 200
          },
          {
            "id": 136,
            "letter": "ن",
            "category": "ثقافة عامة",
            "question": "وش اسم النبتة الخضراء المنعشة التي تُضاف للعصائر والشاي؟",
            "answer": "نعناع",
            "hint": "رائحته منعشة وتُستخدم في المشروبات",
            "points": 200
          },
          {
            "id": 137,
            "letter": "ن",
            "category": "مسلسلات سعودية",
            "question": "وش اسم خدمة البث الرقمي الأمريكية الأشهر التي أنتجت أول مسلسل سعودي؟",
            "answer": "نتفليكس",
            "hint": "تُنتج محتوى محلياً في كل دول العالم",
            "points": 200
          },
          {
            "id": 138,
            "letter": "ن",
            "category": "شبكة المجد",
            "question": "وش اسم البرنامج الشهير على شبكة المجد الذي يستقبل أسئلة المشاهدين الشرعية؟",
            "answer": "نور على الدرب",
            "hint": "برنامج فتاوى يجيب فيه العلماء",
            "points": 400
          }
        ],
        "ه": [
          {
            "id": 139,
            "letter": "ه",
            "category": "تاريخ",
            "question": "وش اسم هجرة النبي ﷺ من مكة للمدينة التي منها بدأ التقويم الإسلامي؟",
            "answer": "الهجرة النبوية الشريفة",
            "hint": "في السنة الأولى للتقويم الهجري",
            "points": 200
          },
          {
            "id": 140,
            "letter": "ه",
            "category": "جغرافيا",
            "question": "وش اسم الهضبة الجغرافية الواسعة التي تقع عليها معظم مناطق وسط المملكة؟",
            "answer": "هضبة نجد",
            "hint": "تتراوح ارتفاعاتها بين 600 و1500 متر",
            "points": 400
          },
          {
            "id": 141,
            "letter": "ه",
            "category": "مسلسلات سعودية",
            "question": "وش اسم المخرجة السعودية الأولى التي أخرجت فيلم وجدة؟",
            "answer": "هيفاء المنصور",
            "hint": "أول فيلم طويل تُخرجه امرأة سعودية",
            "points": 400
          },
          {
            "id": 142,
            "letter": "ه",
            "category": "لهجات سعودية",
            "question": "وش اسم لعبة الورق الشعبية الأشهر في الديوانيات والسهرات الخليجية؟",
            "answer": "هوكم",
            "hint": "تُلعب بأربعة لاعبين بالورق",
            "points": 200
          }
        ],
        "و": [
          {
            "id": 143,
            "letter": "و",
            "category": "جغرافيا",
            "question": "وش اسم أشهر وادٍ تاريخي يمر بقلب الرياض ويُعدّ متنفساً للمدينة؟",
            "answer": "وادي حنيفة",
            "hint": "أُنشئ فيه متنزه وادي حنيفة",
            "points": 200
          },
          {
            "id": 144,
            "letter": "و",
            "category": "إسلاميات",
            "question": "وش اسم الوثيقة التي أبرمها النبي ﷺ مع أهل المدينة لتنظيم العيش المشترك؟",
            "answer": "وثيقة المدينة",
            "hint": "من أقدم الوثائق الدستورية في التاريخ",
            "points": 600
          },
          {
            "id": 145,
            "letter": "و",
            "category": "مسلسلات سعودية",
            "question": "وش اسم الفيلم السعودي الأول الذي أخرجته امرأة وعُرض دولياً؟",
            "answer": "وجدة",
            "hint": "قصة فتاة سعودية تحلم بدراجة هوائية",
            "points": 400
          },
          {
            "id": 146,
            "letter": "و",
            "category": "تقنية",
            "question": "وش اسم أكثر تطبيقات التراسل الفوري استخداماً في العالم؟",
            "answer": "واتساب",
            "hint": "اشترته شركة Meta بمليارات الدولارات",
            "points": 200
          }
        ],
        "ي": [
          {
            "id": 147,
            "letter": "ي",
            "category": "تقنية",
            "question": "وش اسم أشهر موقع لمشاركة ومشاهدة الفيديوهات في العالم؟",
            "answer": "يوتيوب",
            "hint": "Google هي المالكة له",
            "points": 200
          },
          {
            "id": 148,
            "letter": "ي",
            "category": "جغرافيا",
            "question": "وش اسم الدولة الآسيوية الأكثر تقدماً تقنياً وتصنّع السيارات والإلكترونيات؟",
            "answer": "اليابان",
            "hint": "بلاد الشمس المشرقة",
            "points": 200
          },
          {
            "id": 149,
            "letter": "ي",
            "category": "إسلاميات",
            "question": "وش اسم اليوم الذي يُختم فيه الحج بذبح الأضاحي وهو أول أيام عيد الأضحى؟",
            "answer": "يوم النحر",
            "hint": "العاشر من ذي الحجة",
            "points": 400
          },
          {
            "id": 150,
            "letter": "ي",
            "category": "جغرافيا",
            "question": "وش اسم الدولة العربية الجارة للمملكة من الجنوب المعروفة بقهوة المختار؟",
            "answer": "اليمن",
            "hint": "عاصمتها صنعاء",
            "points": 200
          }
        ],
        "ذ": [
          {
            "id": 151,
            "letter": "ذ",
            "category": "لهجات سعودية",
            "question": "وش معنى كلمة 'ذيب' في اللهجة السعودية؟",
            "answer": "ذئب — الحيوان المفترس",
            "hint": "حيوان مشهور في الأمثال العربية",
            "points": 200
          },
          {
            "id": 152,
            "letter": "ذ",
            "category": "تاريخ",
            "question": "وش اسم الملك السعودي الذي يُلقب بـ'ذو النورين' أي النبي العثيم؟",
            "answer": "ذاكرة التاريخ — عثمان بن عفان ذو النورين",
            "hint": "الخليفة الثالث في الإسلام",
            "points": 600
          },
          {
            "id": 153,
            "letter": "ذ",
            "category": "ثقافة عامة",
            "question": "وش اسم الحيوان الصحراوي السريع المشهور بقرونه الطويلة في الجزيرة العربية؟",
            "answer": "ذيب الصحراء — الغزال",
            "hint": "يُشبَّه به الجمال في الشعر العربي",
            "points": 400
          },
          {
            "id": 154,
            "letter": "ذ",
            "category": "إسلاميات",
            "question": "وش اسم الشهر الهجري الذي تُؤدى فيه فريضة الحج؟",
            "answer": "ذو الحجة",
            "hint": "آخر شهور السنة الهجرية",
            "points": 200
          },
          {
            "id": 155,
            "letter": "ذ",
            "category": "ثقافة عامة",
            "question": "وش اسم الجهاز الكهربائي المنزلي الذي يُستخدم لتدفئة الطعام بسرعة؟",
            "answer": "ذرة المايكروويف — مايكروويف",
            "hint": "يعمل بالموجات الكهرومغناطيسية",
            "points": 200
          },
          {
            "id": 156,
            "letter": "ذ",
            "category": "لهجات سعودية",
            "question": "وش معنى 'ذبحتني' في اللهجة السعودية الحديثة؟",
            "answer": "ذبحتني — تعني أتعبتني أو ضايقتني جداً",
            "hint": "تُقال للشخص الذي أرهقك",
            "points": 200
          }
        ],
        "ض": [
          {
            "id": 157,
            "letter": "ض",
            "category": "ثقافة عامة",
            "question": "وش اسم اللغة التي تُلقَّب بـ'لغة الضاد'؟",
            "answer": "ضاد — اللغة العربية",
            "hint": "الضاد حرف فريد خاص بالعربية",
            "points": 200
          },
          {
            "id": 158,
            "letter": "ض",
            "category": "جغرافيا",
            "question": "وش اسم الضفة الغربية في فلسطين وعاصمتها الإدارية؟",
            "answer": "ضفة الأردن — رام الله",
            "hint": "فيها مقر السلطة الفلسطينية",
            "points": 400
          },
          {
            "id": 159,
            "letter": "ض",
            "category": "الدوري السعودي",
            "question": "وش اسم ضربة كرة القدم التي تُسجَّل من نقطة محددة بعد الخطأ في منطقة الجزاء؟",
            "answer": "ضربة جزاء — ركلة الجزاء",
            "hint": "تُنفَّذ من نقطة المركز في منطقة الجزاء",
            "points": 200
          },
          {
            "id": 160,
            "letter": "ض",
            "category": "سيارات",
            "question": "وش اسم الضوء الأمامي الإضافي في السيارة الذي يُستخدم في الضباب؟",
            "answer": "ضوء الضباب",
            "hint": "لونه أصفر وزجاجه مختلف",
            "points": 400
          },
          {
            "id": 161,
            "letter": "ض",
            "category": "لهجات سعودية",
            "question": "وش معنى كلمة 'ضيقة' في اللهجة السعودية الدارجة؟",
            "answer": "ضائقة مالية أو موقف صعب",
            "hint": "تقول 'أنا في ضيقة' لما تكون في ورطة",
            "points": 200
          },
          {
            "id": 162,
            "letter": "ض",
            "category": "ثقافة عامة",
            "question": "وش معنى كلمة 'ضمير' في سياق الأخلاق؟",
            "answer": "ضمير — الصوت الداخلي الذي يميّز الصح من الغلط",
            "hint": "يقال 'فلان عنده ضمير' لمن يلتزم بالأخلاق",
            "points": 200
          }
        ],
        "ظ": [
          {
            "id": 163,
            "letter": "ظ",
            "category": "ثقافة عامة",
            "question": "وش اسم ظاهرة تكوين الماء على السطوح الباردة في الصباح الباكر؟",
            "answer": "ظاهرة الندى",
            "hint": "تتشكل قطرات ماء صغيرة على النباتات صباحاً",
            "points": 200
          },
          {
            "id": 164,
            "letter": "ظ",
            "category": "لهجات سعودية",
            "question": "وش معنى 'ظاهر' في اللهجة السعودية كتعبير عن الرأي؟",
            "answer": "ظاهر — يبدو أو على الأرجح",
            "hint": "تقولها لما تتوقع شيئاً",
            "points": 200
          },
          {
            "id": 165,
            "letter": "ظ",
            "category": "إسلاميات",
            "question": "وش اسم الصلاة الإسلامية التي تُؤدى في منتصف النهار؟",
            "answer": "ظهر — صلاة الظهر",
            "hint": "ثانية الصلوات الخمس",
            "points": 200
          },
          {
            "id": 166,
            "letter": "ظ",
            "category": "ثقافة عامة",
            "question": "وش اسم الحيوان الكبير ذو الرقبة الطويلة الذي يعيش في أفريقيا؟",
            "answer": "ظبي... زرافة",
            "hint": "أطول حيوان على وجه الأرض",
            "points": 400
          },
          {
            "id": 167,
            "letter": "ظ",
            "category": "تاريخ",
            "question": "وش اسم نظام الحكم الذي يُوصَف فيه الملك أو الأمير بأنه 'ظل الله على الأرض'؟",
            "answer": "ظل السلطان — الحكم الملكي المطلق",
            "hint": "مصطلح تاريخي قديم في الدولة الإسلامية",
            "points": 600
          },
          {
            "id": 168,
            "letter": "ظ",
            "category": "جغرافيا",
            "question": "وش اسم مدينة ظفار في جنوب سلطنة عُمان المشهورة بالبخور؟",
            "answer": "ظفار — مدينة صلالة",
            "hint": "تشتهر بالبخور والطبيعة الخضراء في الخريف",
            "points": 400
          }
        ],
        "غ": [
          {
            "id": 169,
            "letter": "غ",
            "category": "ثقافة عامة",
            "question": "وش اسم أول رائد فضاء روسي طار في الفضاء؟",
            "answer": "غاغارين — يوري غاغارين",
            "hint": "في أبريل 1961م حوّل الفضاء لواقع",
            "points": 400
          },
          {
            "id": 170,
            "letter": "غ",
            "category": "لهجات سعودية",
            "question": "وش معنى 'غلا' في اللهجة السعودية؟",
            "answer": "غلا — ارتفع سعره أو أصبح عزيزاً ومحبوباً",
            "hint": "تقولها لما يرتفع سعر الشيء",
            "points": 200
          },
          {
            "id": 171,
            "letter": "غ",
            "category": "إسلاميات",
            "question": "وش اسم الغار الذي اختبأ فيه النبي ﷺ وصاحبه أبو بكر أثناء الهجرة؟",
            "answer": "غار ثور",
            "hint": "في جبل ثور جنوب مكة المكرمة",
            "points": 200
          },
          {
            "id": 172,
            "letter": "غ",
            "category": "جغرافيا",
            "question": "وش اسم مدينة غرب السعودية المشهورة بأسواقها التراثية؟",
            "answer": "غامد — منطقة الباحة",
            "hint": "قبيلة وأرض أصيلة في جنوب غرب المملكة",
            "points": 400
          },
          {
            "id": 173,
            "letter": "غ",
            "category": "سيارات",
            "question": "وش اسم غطاء محرك السيارة الأمامي الذي يُرفع للصيانة؟",
            "answer": "غطاء المحرك — الكبوت",
            "hint": "يُرفع لتفقد الزيت والماء",
            "points": 200
          },
          {
            "id": 174,
            "letter": "غ",
            "category": "ثقافة عامة",
            "question": "وش اسم الغاز الطبيعي المصاحب للنفط الذي تستخدمه المملكة لتوليد الكهرباء؟",
            "answer": "غاز أرامكو — الغاز الطبيعي",
            "hint": "يُستخرج مع النفط من باطن الأرض",
            "points": 400
          }
        ]
      }
    },
    "barra_salfa": {
      "name": "برة السالفة",
      "description": "خمّن التصنيف الذي تنتمي إليه العناصر الستة",
      "totalCategories": 50,
      "categories": [
        {
          "category": "أكلات سعودية شعبية",
          "items": [
            "كبسة",
            "مضغوط",
            "جريش",
            "هريس",
            "مطازيز",
            "معصوب"
          ]
        },
        {
          "category": "أندية الدوري السعودي",
          "items": [
            "الهلال",
            "النصر",
            "الاتحاد",
            "الأهلي",
            "التعاون",
            "الشباب"
          ]
        },
        {
          "category": "مدن سعودية رئيسية",
          "items": [
            "القطيف",
            "جدة",
            "مكة المكرمة",
            "المدينة المنورة",
            "الدمام",
            "أبها"
          ]
        },
        {
          "category": "مطاعم عالمية في السعودية",
          "items": [
            "ماكدونالدز",
            "كنتاكي",
            "برغر كنج",
            "بيتزا هت",
            "ستاربكس",
            "سبواي"
          ]
        },
        {
          "category": "ماركات سيارات يابانية",
          "items": [
            "تويوتا",
            "هوندا",
            "نيسان",
            "مازدا",
            "سوبارو",
            "لكزس"
          ]
        },
        {
          "category": "تطبيقات التواصل الاجتماعي",
          "items": [
            "واتساب",
            "إنستقرام",
            "سناب تشات",
            "تيك توك",
            "تويتر X",
            "يوتيوب"
          ]
        },
        {
          "category": "أنواع التمور السعودية",
          "items": [
            "المجدول",
            "السكري",
            "الخلاص",
            "البرحي",
            "الرزيز",
            "العجوة"
          ]
        },
        {
          "category": "مناطق المملكة العربية السعودية",
          "items": [
            "منطقة الرياض",
            "منطقة مكة المكرمة",
            "المنطقة الشرقية",
            "منطقة عسير",
            "منطقة تبوك",
            "منطقة القصيم"
          ]
        },
        {
          "category": "نجوم كرة القدم في الدوري السعودي",
          "items": [
            "كريستيانو رونالدو",
            "كريم بنزيمة",
            "ريياد محرز",
            "ساديو ماني",
            "مالكوم",
            "جوردان هندرسون"
          ]
        },
        {
          "category": "قنوات مجموعة MBC",
          "items": [
            "MBC1",
            "MBC2",
            "MBC3",
            "MBC4",
            "MBC Masr",
            "MBC Action"
          ]
        },
        {
          "category": "أجهزة إلكترونية منزلية",
          "items": [
            "تلفزيون",
            "ثلاجة",
            "غسالة",
            "مكيف",
            "مايكروويف",
            "فرن"
          ]
        },
        {
          "category": "مشاهير الكوميديا السعودية",
          "items": [
            "ناصر القصبي",
            "عبدالله السدحان",
            "حمد الفريدي",
            "علي الهليل",
            "عبدالله مشعل",
            "عمر الشيخ"
          ]
        },
        {
          "category": "مواقع سياحية في السعودية",
          "items": [
            "العُلا",
            "مدائن صالح",
            "الدرعية",
            "الحنيفة",
            "جزر فرسان",
            "واجهة جدة البحرية"
          ]
        },
        {
          "category": "كلمات نجدية دارجة",
          "items": [
            "سالفة",
            "عدل",
            "يشبع",
            "هوبي",
            "دلعة",
            "صج"
          ]
        },
        {
          "category": "مشروبات سعودية شعبية",
          "items": [
            "قهوة عربية",
            "شاي بالنعناع",
            "عصير تمر هندي",
            "لبن",
            "عصير مانجو",
            "قمر الدين"
          ]
        },
        {
          "category": "شركات تقنية عالمية",
          "items": [
            "آبل",
            "سامسونج",
            "مايكروسوفت",
            "جوجل",
            "أمازون",
            "ميتا"
          ]
        },
        {
          "category": "أنبياء مذكورون في القرآن الكريم",
          "items": [
            "إبراهيم عليه السلام",
            "موسى عليه السلام",
            "عيسى عليه السلام",
            "يوسف عليه السلام",
            "نوح عليه السلام",
            "داود عليه السلام"
          ]
        },
        {
          "category": "رياضات شعبية في السعودية",
          "items": [
            "كرة القدم",
            "الصقارة",
            "سباق الهجن",
            "كرة السلة",
            "التنس",
            "رياضة الفروسية"
          ]
        },
        {
          "category": "برامج رمضانية شهيرة",
          "items": [
            "طاش ما طاش",
            "سيلفي",
            "رامز في الغابة",
            "Arab Idol",
            "ذا فويس",
            "عرب غوت تالنت"
          ]
        },
        {
          "category": "سيارات فارهة عالمية",
          "items": [
            "رولز رويس",
            "بنتلي",
            "لامبورغيني",
            "فيراري",
            "ماكلارين",
            "أستون مارتن"
          ]
        },
        {
          "category": "حلويات خليجية شعبية",
          "items": [
            "لقيمات",
            "محلبية",
            "أم علي",
            "بسبوسة",
            "زلابية",
            "كنافة"
          ]
        },
        {
          "category": "محافظات منطقة الرياض",
          "items": [
            "الزلفي",
            "المجمعة",
            "الأفلاج",
            "الدوادمي",
            "الخرج",
            "رماح"
          ]
        },
        {
          "category": "لاعبو المنتخب السعودي الأبطال",
          "items": [
            "سالم الدوسري",
            "محمد العويس",
            "علي البليهي",
            "عبدالحميد القادري",
            "يحيى الشهراني",
            "سامي الجابر"
          ]
        },
        {
          "category": "برامج شبكة المجد التلفزيونية",
          "items": [
            "نور على الدرب",
            "فقه الحياة",
            "مجلس العلماء",
            "الدين والحياة",
            "مع القرآن",
            "فتاوى رمضان"
          ]
        },
        {
          "category": "مشاريع رؤية 2030 الكبرى",
          "items": [
            "نيوم",
            "ذا لاين",
            "القدية",
            "البحر الأحمر",
            "عاصمة المستقبل تروجينا",
            "بيبان المنتدى"
          ]
        },
        {
          "category": "البهارات والتوابل الشعبية",
          "items": [
            "كركم",
            "قرفة",
            "كمون",
            "دارسين",
            "فلفل أسود",
            "بهار مشكل"
          ]
        },
        {
          "category": "كلمات لهجة حجازية",
          "items": [
            "تمام",
            "ياخي",
            "حبة",
            "دوشة",
            "فنك",
            "حبة بعدين"
          ]
        },
        {
          "category": "معالم الرياض السياحية",
          "items": [
            "برج المملكة",
            "قصر المصمك",
            "حديقة الحيوان",
            "وادي حنيفة",
            "متحف الرياض",
            "البوليفارد رياض سيتي"
          ]
        },
        {
          "category": "شركات الاتصالات السعودية",
          "items": [
            "STC الاتصالات السعودية",
            "موبايلي",
            "زين",
            "فيرجن موبايل",
            "stc tv",
            "اتصالات السعودية الأعمال"
          ]
        },
        {
          "category": "أنواع الأرز العالمية",
          "items": [
            "أرز بسمتي",
            "أرز ياباني",
            "أرز كالروز",
            "أرز أربوريو",
            "أرز بني",
            "أرز الياسمين"
          ]
        },
        {
          "category": "مهرجانات ومواسم سعودية",
          "items": [
            "موسم الرياض",
            "موسم جدة",
            "موسم الطائف",
            "تطوير موسم العُلا",
            "مهرجان الجنادرية",
            "موسم الدرعية"
          ]
        },
        {
          "category": "علماء سعوديون معاصرون",
          "items": [
            "ابن باز رحمه الله",
            "ابن عثيمين رحمه الله",
            "الألباني رحمه الله",
            "صالح الفوزان",
            "عبدالله المطلق",
            "سعد الشثري"
          ]
        },
        {
          "category": "معالم مدينة جدة",
          "items": [
            "نافورة الملك فهد",
            "كورنيش جدة",
            "حي البلد التاريخي",
            "برج المملكة جدة",
            "سوق الجوهرة",
            "مطار الملك عبدالعزيز"
          ]
        },
        {
          "category": "أنواع الخبز السعودي",
          "items": [
            "خبز التنور",
            "مرقوق",
            "خبز الصاج",
            "رقاق",
            "قرصان",
            "كبانا"
          ]
        },
        {
          "category": "يوتيوبرز سعوديون مشهورون",
          "items": [
            "بدر صالح",
            "عمر يوسف",
            "بلال رؤوف",
            "أبو فلة",
            "واجد ابن الشيخ",
            "مشعل اللهيبي"
          ]
        },
        {
          "category": "أسماء أسماك الخليج العربي",
          "items": [
            "زبيدي",
            "هامور",
            "شعم",
            "قباب",
            "سكن",
            "كنعد"
          ]
        },
        {
          "category": "لاعبو كرة القدم البرازيليون في السعودية",
          "items": [
            "نيمار",
            "روبرتو فيرمينو",
            "مارسيلو",
            "أندرسون تاليسكا",
            "جيسوس",
            "فيليبي"
          ]
        },
        {
          "category": "مشاريع الترفيه والسياحة",
          "items": [
            "حديقة المشاعل",
            "ملاهي الرياض بارك",
            "موسم أبوظبي",
            "إكسبو 2030",
            "سيزر بالاس الرياض",
            "تروجينا الرياضات الشتوية"
          ]
        },
        {
          "category": "عطور ومواد البخور العربية",
          "items": [
            "عود هندي",
            "مسك",
            "عنبر",
            "دهن العود",
            "بخور دلال",
            "ورد طائفي"
          ]
        },
        {
          "category": "شخصيات كرتونية يحبها الأطفال",
          "items": [
            "باباو",
            "سبونج بوب",
            "شاشة توم وجيري",
            "فيلبس",
            "طيور الجنة",
            "براعم"
          ]
        },
        {
          "category": "صحف ومجلات سعودية شهيرة",
          "items": [
            "عكاظ",
            "الجزيرة",
            "الرياض",
            "سعودي غازيت",
            "المدينة",
            "البلاد"
          ]
        },
        {
          "category": "ماركات الهواتف الذكية العالمية",
          "items": [
            "آيفون",
            "سامسونج غالاكسي",
            "هواوي",
            "شاومي",
            "أوبو",
            "ون بلس"
          ]
        },
        {
          "category": "ملوك المملكة العربية السعودية",
          "items": [
            "الملك عبدالعزيز",
            "الملك سعود",
            "الملك فيصل",
            "الملك خالد",
            "الملك فهد",
            "الملك عبدالله"
          ]
        },
        {
          "category": "أنواع الغتر والأقمشة التقليدية",
          "items": [
            "غترة بيضاء نجدية",
            "شماغ أحمر وأبيض",
            "طاقية بيضاء",
            "عقال أسود",
            "بشت فاخر",
            "ثوب مخطط زري"
          ]
        },
        {
          "category": "مشاهير الغناء والطرب السعودي",
          "items": [
            "محمد عبده",
            "طلال مداح",
            "راشد الماجد",
            "عبادي الجوهر",
            "أبو بكر سالم",
            "محمد اليامي"
          ]
        },
        {
          "category": "أنواع حلويات العيد السعودية",
          "items": [
            "كليجة",
            "مطبق",
            "شعيبيات",
            "عيش السرايا",
            "حلوى شعبية",
            "تمر بالقشطة"
          ]
        },
        {
          "category": "أماكن التسوق في الرياض",
          "items": [
            "العثيم مول",
            "الرياض بارك",
            "مول العرب",
            "بانوراما مول",
            "البوليفارد",
            "ريم مول"
          ]
        },
        {
          "category": "وصفات القهوة العربية",
          "items": [
            "ماء بارد للتبريد",
            "دلة نحاسية تقليدية",
            "فناجيل خزفية صغيرة",
            "تقليب بثلاثة أصابع",
            "قهوة خفيفة مرّة",
            "ضيافة التمر مع القهوة"
          ]
        },
        {
          "category": "مواصفات الضيافة النجدية",
          "items": [
            "القهوة العربية",
            "التمر",
            "الفطور الكريم",
            "البخور",
            "الترحيب بالضيف",
            "المقهى أو المجلس"
          ]
        },
        {
          "category": "شركات طيران تعمل في السعودية",
          "items": [
            "الخطوط السعودية",
            "فلاي ناس",
            "طيران أديل",
            "الإمارات",
            "قطر إيرويز",
            "فلاي دبي"
          ]
        }
      ]
    }
  }
};
